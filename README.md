...
- **🏠 Your data, on your hardware.** Everything Loom keeps lives on your machine — an **SQLite** store,
  your git checkouts, your transcripts, and your vault. Loom adds **no cloud service of its own** — there's no Loom account and no Loom server, and your code, transcripts and history are never uploaded anywhere. The daemon binds to **loopback** (`127.0.0.1`) by default, and every UI write route behind it requires a **local access credential** — a bearer secret generated at boot and kept `0600` under `LOOM_HOME` — so another process on the same machine can't drive the API just by reaching the socket. Opening it to another device is deliberate and opt-in (see [Reach Loom from another device](#reach-loom-from-another-device)).

  That's not the same as "nothing leaves the machine", so here is the whole list. **On by default**, beyond the agents' own conversations with Anthropic: Loom polls `api.anthropic.com/api/oauth/usage` **every 60s** with your Claude OAuth token to draw the plan-usage meters (set `LOOM_SUPPRESS_USAGE_POLLER=1` to stop it); an **npm-installed** daemon polls `registry.npmjs.org` **every 6h** for the `loomctl` dist-tags that drive the update banner (a from-source daemon never makes this call, and there's no flag to disable it on a packaged one — `LOOM_NPM_REGISTRY` only redirects it); and creating a worker's worktree runs your own package manager's install in it, which reaches whatever registry that project is configured for. **Opt-in, off until you configure them:** the Telegram companion, outbound alert webhooks, and any Connection you bind.
