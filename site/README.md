...
- `remote-access.html` — a docs subpage covering how to reach the daemon from another device
  ...
  TLS as the alternative; a section documents Tailscale Serve and other reverse proxies through the trusted-proxy listener — read-and-steer,
  gateway-token-gated, unverified on a live tailnet). Reuses `styles.css` and the same inline reveal/copy JS; linked from
