...
//   - obsidianEnabled=true → ONLY pickup/session-end get the Obsidian "vault preflight" fragment appended
...
try {
  ...
  check("(a) off: pickup byte-identical to store base", injected(repoOff, "pickup") === BASE["pickup"]);
  check("(a) off: session-end byte-identical to store base", injected(repoOff, "session-end") === BASE["session-end"]);
  check("(a) off: no fragment text anywhere in pickup", !injected(repoOff, "pickup").includes("Vault preflight"));
  ...
  check("(a) default arg (omitted) == off: pickup byte-identical", injected(repoDefault, "pickup") === BASE["pickup"]);

  // (b) ON → pickup/session-end END with the fragment, frontmatter intact.
  injectSkills(repoOn, "sess-on", null, null, true);
  check("(b) on: pickup == base + separator + real fragment", injected(repoOn, "pickup") === expectedWithFragment("pickup"));
  check("(b) on: session-end == base + separator + real fragment", injected(repoOn, "session-end") === expectedWithFragment("session-end"));
  check("(b) on: pickup ends with the fragment", injected(repoOn, "pickup").endsWith(FRAGMENT));
  check("(b) on: pickup frontmatter block intact at the top", injected(repoOn, "pickup").startsWith("---\nname: pickup\ndescription: P\n---\n"));
  check("(b) on: fragment lands AFTER the body (base body precedes the fragment)", injected(repoOn, "pickup").indexOf("Read the board.") < injected(repoOn, "pickup").indexOf("Vault preflight"));
  ...
  injectSkills(repoOn, "sess-on", null, null, true);
  check("re-inject on: pickup still has exactly ONE fragment (append not doubled)", injected(repoOn, "pickup") === expectedWithFragment("pickup"));
  ...
  check("flip on→off: pickup restored byte-identical to base", injected(repoOn, "pickup") === BASE["pickup"]);
} finally {
