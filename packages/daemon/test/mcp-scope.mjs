...
// tasks_* (the project-scoped board, incl. the card 0d4bc3f0 deferred-hand-off pair
// tasks_defer_item/tasks_defer_item_ack) + wake_* (the self-scheduled wake_me primitive) + memory_* (the
// project-scoped shared agent memory store) + task_request_get/task_requests_list (read a task's
// connected Requests) — all ride the per-session loom-tasks surface. Keep this list in sync as the
// tasks-MCP tool surface grows.
const expectedTaskTools = "memory_forget,memory_list,memory_read,memory_write,task_request_get,task_requests_list,tasks_create,tasks_defer_item,tasks_defer_item_ack,tasks_get,tasks_list,tasks_update,wake_cancel,wake_list,wake_me";
