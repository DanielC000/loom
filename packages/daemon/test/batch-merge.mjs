...
// BATCH MERGE GATE — CORE LANDING SHAPES + PATHSET/BASE STAMPING (card dbc6f660 — gate N ready branches
// once; card 6801c0a1 — land each branch's OWN commits INDIVIDUALLY, never squashed, only the branch's
// LAST commit carrying the Loom-Worker-Branch trailer — see git/batch-merge.ts's own header doc for the
// full correction).
...
//
// SPLIT FROM ONE FILE (card b5af744d, successor to diagnosis card 53a818eb): this file used to also carry
// scenarios (7d) through the trailing no-op-fast-forward case, running 88.4s solo against the blanket
// 120_000ms TEST_TIMEOUT_MS (1.36x headroom) — no single dead fixed cost, just pure git-volume content
// growth spread ~evenly across ~19 scenarios (profiled per-scenario before splitting; see this card's
// worker report for the full breakdown). The remaining scenarios — PathSet-forgery rejection, rename
// robustness, Claude-Session trailer stripping, stamp-failure surfacing, empty-commit fail-closed,
// redundant-commit-inside-a-multi-commit-branch, and sort-by-author-date — now live in the sibling file
// `batch-merge-robustness.mjs`. Every scenario from the original file still runs, just split across the
// two files by group; this file covers the core landing mechanics + the PathSet/Base stamp basics.
//
// Proves:
...
//   (7f) a multi-commit contribution verified against sha^ (Base trailer lost/stripped) fails closed.
//   (7g) author identity + date are preserved across the follow-up PathSet/Base amend.
// Also covers computeBatchSize's fixed K = min(ready, maxWorkers) rule.
//
// See batch-merge-robustness.mjs for: (7d) forged-digest rejection, (7e) rename robustness, (8)
// Claude-Session trailer stripping, (9)/(9b) stamp-failure vs. noop, (10) empty-commit fail-closed, (11)/
// (11b) redundant-commit-inside-a-multi-commit-branch, (12) sort-by-author-date, and the trailing no-op
// fast-forward case.
//
...
// WORKTREES_DIR as a SIBLING of LOOM_HOME (paths.ts) — a bare `node batch-merge.mjs` run with no LOOM_HOME
// set would otherwise leak real worktrees into the owner's real ~/.loom-worktrees. useOwnLoomHome reuses
// the harness's own per-test temp home when run via scripts/test-daemon.mjs (which already sets one), or
// mkdtemp's + registers its own when run bare directly — either way LOOM_HOME is a temp dir BEFORE the
// dist import below, and requireHermeticEnv() makes that fail closed rather than silently leak again.
useOwnLoomHome("loom-bm-parent-");
requireHermeticEnv();
