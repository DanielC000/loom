...
// ============================ (T) TOOL SURFACE: registration seam ============================
{
  ...
  // Anchor the gate: the worker surface is exactly { gate_cancel, gate_queue, gate_status, my_context,
  // run_gate, worker_report } (depth-1 tree holds at the surface). my_context is the own-occupancy self-assessment
  ...
  // read-only, own-lineage-scoped delivery-history complement; gate_cancel (card a0d912f5) is a WRITE but
  // scoped to the caller's own gateType:"worker" op only — all added since this anchor was first written.
  check("(T) worker surface is exactly { directive_status, gate_cancel, gate_queue, gate_status, my_context, run_gate, worker_report }",
