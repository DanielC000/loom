...
// contention apart from a leaked slot in one read instead of guessing) + gate_cancel (card 8d585277, the
// manual withdraw/supersede escalation for a queued or running worker self-check — project-scoped,
// gateType-scoped to `worker` in both phases; merge/deploy are refused, never silently no-op'd) + the
// manager self-management tools (daemon_restart self-deploy, served_status for post-restart deploy
// verification, end_me self-scoped terminal exit — card 3b015fc7, idle_report for the asleep-at-the-wheel
// watcher, inbox_pull fast-drain, recycle_me for context-recycle) + the manager self-service management
// surface (agent_list read-only directory, agent_get single-record read, agent_assign_profile/agent_update,
...
// 5) ROLE-BASED surface: a WORKER connects but sees ONLY [directive_status, gate_cancel, gate_queue,
//    gate_status, my_context, run_gate, worker_report] (no manager coordination tools — the depth-1 tree
//    holds at the surface, not just a gate). my_context is the own-occupancy self-assessment tool,
//    available to any role; worker_report is NOT a manager tool; run_gate (card 7f96aa09) is the
//    daemon-mediated DoD self-gate; gate_status (card fc243a43) is the read-only, own-op-scoped complement
//    to run_gate; gate_queue (card d04f9c76) is the read-only, project-scoped daemon-wide queue snapshot;
//    directive_status (card 35c96aa6) is the read-only, own-lineage-scoped delivery-history complement to
//    the manager's staleDirectiveProjection; gate_cancel (card a0d912f5) is a WRITE, but scoped to
//    cancelling only the caller's own run_gate self-check — all added since this check was written.
