...
process.env.LOOM_HOME = path.join(os.tmpdir(), `loom-periodic-snap-${Date.now()}-${process.pid}`);
...
const sandboxHome = path.join(os.tmpdir(), `loom-periodic-snap-home-${Date.now()}-${process.pid}`);
...
const fakeCwd = path.join(os.tmpdir(), `loom-periodic-snap-cwd-${Date.now()}-${process.pid}`);
