...
// 7) ROLE GATE — a WORKER connects but sees ONLY {gate_cancel, gate_queue, gate_status, my_context,
//    run_gate, worker_report}; NONE of the management tools. (This assertion was already stale before card 7f96aa09
...
//    read-only project-scoped daemon-wide queue snapshot, and directive_status, card 35c96aa6's read-only
//    own-lineage-scoped delivery-history complement, and gate_cancel, card a0d912f5's own-op-scoped write
//    (cancel only your own run_gate self-check), are the latest additions.)
const W = await connect("mcp-orch", "W");
const wTools = (await W.listTools()).tools.map((t) => t.name).sort();
check(`role-gate: worker sees ONLY [directive_status, gate_cancel, gate_queue, gate_status, my_context, run_gate, worker_report] (got ${wTools.join(",")})`, wTools.join(",") === "directive_status,gate_cancel,gate_queue,gate_status,my_context,run_gate,worker_report");
