...
//    own-lineage-scoped delivery-history complement, are the latest additions.)
const W = await connect("mcp-orch", "W");
const wTools = (await W.listTools()).tools.map((t) => t.name).sort();
check(`role-gate: worker sees ONLY [directive_status, gate_queue, gate_status, my_context, run_gate, worker_report] (got ${wTools.join(",")})`, wTools.join(",") === "directive_status,gate_queue,gate_status,my_context,run_gate,worker_report");
