...
commitAll(repo, "init", "-c user.email=p5@loom -c user.name=p5");
...
try {
  ...
  // cross-project requests_list, card 59489267) + the THREE narrow writes (audit_file_finding +
  // preset_suggestion_suggest + the self-scoped end_me) — and NONE of the elevated/structural ones. The
  // baseline is extended by EXACTLY the new repo reads + requests_list (no surface creep); the `forbidden`
  // negative control below proves nothing else (no write/host/outward) slipped in.
  const tools = (await client.listTools()).tools.map((t) => t.name).sort();
  check(`(b) audit surface is EXACTLY [audit_file_finding, end_me, list_sessions, preset_suggestion_suggest, repo_glob, repo_grep, repo_read_file, requests_list, transcript_read] (got: ${tools.join(",")})`,
    JSON.stringify(tools) === JSON.stringify(["audit_file_finding", "end_me", "list_sessions", "preset_suggestion_suggest", "repo_glob", "repo_grep", "repo_read_file", "requests_list", "transcript_read"]));
