...
//
// SPLIT FROM a single merge-gate-reuse.mjs (card d627f0fd): the combined file ran 93-184s solo across 4
// runs on a busy self-hosting host (one earlier single-sample read was 111.3s), well past the blanket
// 120_000ms TEST_TIMEOUT_MS with two of four runs already over the ceiling — no override, per doctrine.
// Per-scenario Date.now() profiling (throwaway instrumented copy, discarded before commit — see this
// card's worker report for the full A-T breakdown) found no single dominant scenario or hoistable fixed
// cost (process startup before the first scenario was under 1s of the ~95-96s total): the cost is real git
// I/O spread across 20 scenarios. Split at the natural thematic AND cumulative-time boundary between the
// direct/union-merge path (A-L, ~50s solo) and the preLanded/admission-time/concurrency/guard-confinement
// path (M-T, ~45s solo) — this file carries A-L; see merge-gate-reuse-admission.mjs for M-T. All 140
// original checks (78 + 62) still run, zero deleted.
//
// Proves:
...
//   (G) A NEW COMMIT on the branch itself after the self-check — forces a real re-gate (never matches on a
//       newer sha than the one the self-check actually validated), correctly attributed to the worktree,
//       not the union-merge (main is untouched here).
...
//   (I) THE TOCTOU GUARD ITSELF — mergeBranch's own in-lock re-verification (`requireCanonicalHead`)
//       catches a stale reuse premise INSIDE its own canonical-index lock, before touching anything,
//       rather than trusting a decision made before the lock was even requested.
//   (J) MAIN ADVANCES DURING A REAL (non-reused) GATE'S OWN RUN — card eda70da6. The squash refuses (a
//       benign, retryable race, zero side effects) rather than landing on a tree the gate never actually
//       validated; the retry then lands cleanly once main holds still.
//   (K) MAIN ADVANCES DURING THE SEMAPHORE QUEUE WAIT (between the union-merge and gate admission) — card
//       eda70da6, updated by card b798e706. Admission-time re-derivation absorbs the move; the merge lands
//       on its first pass with no wasted second gate run.
//   (L) THE MOVED-MAIN-BETWEEN-THE-TWO-READS CASE — card 24cc40f9. Reading main's HEAD ONCE and threading
//       that SAME sha into both `countCommitsBehind` and `gateBaseMainHead` closes the TOCTOU window
//       between two independent git reads that used to let a stale premise wrongly match at squash time.
//
// See merge-gate-reuse-admission.mjs for: (M)/(N) the preLanded discriminating pair, (O) admission-time
// re-union conflict, (P) the fail-closed guard firing after a successful admission-time re-union, (Q) two
// genuinely concurrent same-repo confirms, (R)/(S) the per-repo admission guard's leak-proofing and
// confinement, and (T) multiple reuse conditions failing simultaneously.
//
...
console.log(failures === 0
  ? "\n✅ ALL PASS — a merge whose inputs are provably identical to a green, current self-check skips the redundant gate run (gateRan:false + reusedOpId), while a moved main, a stale base, a racy/UNVERIFIED settle, a dirty worktree, a superseding later failure, a new commit on the branch itself, a simulated daemon restart, a real gate's own mid-run main advance, a queue-wait main advance, or the moved-main-between-the-two-reads case ALL still force a real re-gate (gateRan:true, no reusedOpId) or a fail-closed refusal. See merge-gate-reuse-admission.mjs for the preLanded, admission-time, concurrency, and guard-confinement scenarios."
  : `\n❌ ${failures} FAILURE(S).`);
