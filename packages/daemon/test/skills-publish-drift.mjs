...
// otherwise identical reads customized:false; a real edit reads customized:true. (2) publishSkillToBundled
// writes store→asset, restricted to names that already exist as a bundled asset. (3) regression guard:
// right after reset OR publish, customized is false. (Drift here is the `customized` boolean from the new
// 3-version state model — with no base file seeded, base is treated as == shipped, so customized == the
// old store≠asset "diverged". The dedicated state-matrix/merge/adopt coverage lives in skills-customization.mjs.)
...
const root = path.join(os.tmpdir(), `loom-skills-publish-${Date.now()}-${process.pid}`);
...
const customized = (name) => listSkills().find((s) => s.name === name)?.customized;
...
try {
  ...
  check("line-ending/whitespace-only difference reads customized:false", customized("loom-x") === false);

  // A real content edit → customized:true.
  writeSkill("loom-x", "---\nname: loom-x\ndescription: X\n---\n\n# loom-x\n\nEDITED in the UI.\n");
  check("a genuine store edit reads customized:true", customized("loom-x") === true);
  ...
  check("after publish, customized:false (regression guard — base advanced to the published edit)", customized("loom-x") === false);
  ...
  check("edit before reset reads customized:true", customized("loom-x") === true);
  check("resetSkillToBundled returns true", resetSkillToBundled("loom-x") === true);
  check("after reset, customized:false (regression guard)", customized("loom-x") === false);
  ...
  check("non-bundled skill carries no customized flag", customized("local-y") === undefined);
