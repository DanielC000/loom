...
const { computeBootMode } = await import("../dist/pty/host.js");
...
console.log(failures === 0
  ? "\n✅ ALL PASS — computeBootMode picks the session's real target mode DIRECTLY as the boot `--permission-mode` value whenever that target is itself acceptEdits/plan/auto, so the common auto-target boot (fresh worker/manager default, and a resumed session of the same config) never transits an intermediate mode — while a target that isn't directly expressible (default/bypassPermissions/unknown, or a genuinely absent target) falls back to the pre-fix raw permission.mode unchanged, leaving the existing Shift+Tab convergence loop as the sole path for those rare cases."
  : `\n❌ ${failures} FAILURE(S).`);
