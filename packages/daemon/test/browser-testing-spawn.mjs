...
//   plus (env) scratchDirEnv sets LOOM_SCRATCH_DIR == sessionScratchDir (== the Playwright --output-dir
//       for a browserTesting spawn) — and, since card 5d8888b6, for EVERY OTHER spawn too (widened from
//       the original browser-testing-only gate; sessionId alone determines the value now).
//
...
// ===================== (env) LOOM_SCRATCH_DIR — every session's scratch pointer, not just browser (card 5d8888b6) =====================
...
// where --output-dir points (sessionScratchDir). scratchDirEnv is now UNCONDITIONAL (widened from the
// original browser-testing-only gate) — every OTHER session gets the same pointer too, for a repo-external
// place to stage a throwaway file (e.g. Python/document-conversion tooling).
const scratchOn = scratchDirEnv("s1");
...
// THE ARM THAT COULD ACTUALLY DISCRIMINATE THE FIX: a plain (non-browser) session — a browser spawn would
// have passed this check before AND after the fix, so it proves nothing about the widening on its own.
check("(env) plain (non-browser) spawn: LOOM_SCRATCH_DIR IS present (widened — no longer browser-only)",
  scratchDirEnv("s2").LOOM_SCRATCH_DIR === sessionScratchDir("s2"));
check("(env) NOT gated on any mcp-config shape any more — sessionId alone determines the value",
  JSON.stringify(scratchDirEnv("s1")) === JSON.stringify(scratchOn));
...
commitAll(repo, "init", "-c user.email=bt@loom -c user.name=bt");
