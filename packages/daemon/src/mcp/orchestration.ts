    ...
    server.registerTool(
      "merge_batch",
      {
        description: "PRE-MERGE TITLE CHECK (card a32533a1): this call NEVER rewrites or coerces a candidate's commit subject — each branch's own commit(s) land verbatim (see below). Before calling this, check `worker_merge`'s `ownTipSubject`/`ownTipSubjectConventional` fields for each candidate — that's the exact, uncoerced subject for the branch's TIP commit only, byte-for-byte what this call will commit for it. For a branch with a SINGLE commit (the common case) the tip IS the branch's whole contribution, so that field alone is a complete check. For a branch with MORE THAN ONE commit it is NOT — every commit on that branch lands with its own original subject. `worker_merge`'s result also carries `ownNonTipCommitSubjects` (card 591906ae) — every one of THOSE other commits' own subjects, bounded — plus `ownNonTipCommitSubjectsConventional`; check both before batching a multi-commit candidate (no need to hand-run `git log` yourself). A branch that fails this check merges solo instead — the solo squash path rewrites the subject to the card title, so a bad worker-authored subject never reaches mainline that way. Card dbc6f660 — batch the merge gate: gate up to the project's configured `orchestration.maxConcurrentWorkers` (Settings UI: \"Max workers / manager\", resolved fresh via resolveConfig, platform default 3) READY worker branches on ONE repo in a SINGLE run, instead of one gate per branch. This cap is FIXED, not a parameter of this tool — batch size is deliberately not agent-settable. Cuts a dedicated batch worktree off canonical main's current tip and lands each candidate branch into it IN ORDER, EACH BRANCH'S OWN COMMITS INDIVIDUALLY (card 6801c0a1 — a rebase/cherry-pick, NOT a squash: a branch contributing 3 commits lands 3 commits on main, not 1; this is DIFFERENT from a solo worker_merge_confirm, which still squashes a branch to one commit). No merge commits are ever created. Only the branch's LAST (tip) commit carries the `Loom-Worker-Branch` trailer — its earlier commits land with their own original messages and no trailer at all. Runs the gate ONCE on the assembled tree, and on green fast-forwards canonical main to it (main is mutated exactly once, at that fast-forward). Every workerSessionId must be YOUR OWN worker, on the SAME repo — a mismatch refuses the whole call before touching anything. Fewer than 2 eligible candidates (after stranded-work/cap filtering) is not worth batching and falls straight to the ordinary per-branch path. A branch that won't land cleanly into the batch (a real conflict on any of its own commits against an earlier candidate in this SAME batch, or a branch whose own commit range contains a merge commit — e.g. a stale-base auto-forward — which this individual-commit landing cannot cherry-pick) is DROPPED WHOLESALE, never partially landed — recorded in `fallback` with its reason — and the batch proceeds with the rest; the dropped branch then gets its own ordinary worker_merge_confirm (which still squashes, unaffected). A RED gate on the assembled batch is NEVER bisected — every original candidate falls back to being gated individually, exactly today's behavior (this is measured cheaper than bisection at this batch-size range). A FORFEIT (canonical main advanced between the batch being cut and the fast-forward — a human or another writer landed something mid-gate) is also a full fallback to individual gating, and is recorded as a distinct `batch_merge_forfeited` event — this is the one failure mode batching makes strictly worse than today (one branch's gate wasted becomes up to the cap), so watch for it if you batch large groups often. `landed` names every branch that actually merged via the shared batch gate (workerSessionId, taskId, branch, sha, strippedTrailerCount — `sha` is the branch's TIP commit, the one carrying its trailer; `strippedTrailerCount` (card b7f965d2) is how many of THAT branch's own commits carried a `Claude-Session:` harness attribution trailer that got stripped before landing — 0 in the ordinary case, non-zero means a worker put that trailer on its own commit against this project's CLAUDE.md, worth a look) — each one already went through the SAME finalize path (worktree cleanup, task move, notification) a solo ALREADY_MERGED confirm uses, no separate action needed. `fallback` names every OTHER candidate (dropped, over the cap, stranded, or — on a red gate/forfeit — the whole batch) that this call already routed through an ordinary worker_merge_confirm on your behalf — you do not need to re-call it yourself, just read `fallback` to see what happened to each. `reason` is set on a top-level refusal (validation failure, nothing eligible, no gateCommand configured) or to explain why the whole batch went to fallback. " +
          "CLIENT TIMEOUT ≠ FAILURE (card 46ebdf20): this call has no bounded/pending fast-return the way `worker_merge_confirm` does — it awaits the whole batch (worktree cut, assembly, gate, fast-forward) synchronously, so your own client can time out well before the op settles here; that timeout does NOT mean the batch failed. To check on it, read `gate_queue` for a `merge` row with `taskId:null`/`branch:null`/`workerLabel:\"Orchestrator\"` — a solo merge always carries a real `taskId`/`branch`, so those nulls (not their absence) are the discriminator for a live batch. Do NOT re-fire `merge_batch` with the same workerSessionIds to check — unlike `worker_merge_confirm`'s cheap re-poll, this call has no dedupe/attach: a re-fire mints a brand-new op, cuts a whole new batch worktree, and re-assembles every candidate branch before it's even admitted. It will not double-land — a pre-existing per-repo merge-admission guard serializes it behind the first call, which then forfeits cleanly once it sees canonical main already advanced — but it duplicates real, avoidable work every time. (Derived from a code-path read, not exercised against live branches.)",
