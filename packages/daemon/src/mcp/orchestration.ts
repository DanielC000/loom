...
export class OrchestrationMcpRouter {
  ...
  private buildServer(sessionId: string, role: SessionRole): McpServer {
      ...
      async ({ taskId, agentId, kickoffPrompt }) => {
        ...
        try {
          ...
          const dirty = worker.reusedDirtyWorktree
            ...
            : {};
          return ok({ workerSessionId: worker.id, branch: worker.branch, worktreePath: worker.worktreePath, ...shipped, ...dirty });
        } catch (e) {
    ...
    // browserTesting/documentConversion/openDesign/restrictedTools are the SAME resolveProfile output profile_get/
    ...
    server.registerTool(
      "agent_list",
      {
        description:
          ...
          "browserTesting, documentConversion, openDesign, restrictedTools (resolved from the assigned/default " +
