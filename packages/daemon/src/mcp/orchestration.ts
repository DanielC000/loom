...
function registerGateStatus(server: McpServer, sessions: SessionService, scopeSessionId?: string, getScopeProjectId?: () => string | undefined, getRedactOutputFileCallerProjectId?: () => string | undefined): void {
  ...
  const description = forWorker
    ...
    : "Read-only status for ONE merge-gate run, by the `opId` a `worker_merge_confirm` " +
      ...
      "coincidence. `retryDeclineReason` (card 67030bb9) is present ONLY when a genuine gate failure was " +
      "eligible for this retry mechanism to even CONSIDER (never on a `\"kill\"`/`\"timeout\"` attempt 1, " +
      "which the sibling TRANSIENT-KILL retry owns instead) but `identifyRetriableTestFiles` declined it — " +
      "one of its own decline-reason strings (e.g. `\"over-cap\"`, `\"harness-not-executed\"`, " +
      "`\"unparseable-name\"`) explaining WHY, so a rejection can be diagnosed after the fact from " +
      "`gate_status`/`gate_history` instead of needing a fresh live repro. Mutually exclusive with a " +
      "non-null `retriedFile` on the same row — a retry is either identified or declined, never both. " +
