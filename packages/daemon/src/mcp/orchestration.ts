...
import type { FreshMintInfo, PendingOpView } from "../orchestration/pending-ops.js";
    ...
    server.registerTool(
      ...
      async ({ workerSessionId, forceRemoveWorktree }) => {
        try {
          ...
          if (!r.ok) return ok({ error: r.error instanceof Error ? r.error.message : String(r.error) });
          return ok({ ...r.value, ...freshMintFields });
        } catch (e) {
          return ok({ error: (e as Error).message });
        }
      },
