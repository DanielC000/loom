...
export class OrchestrationMcpRouter {
  ...
  private buildServer(sessionId: string, role: SessionRole): McpServer {
    ...
    // from its own worker_message OR worker_redirect call, whichever is MOST RECENT (card 0fbb0507 widened
    // this from worker_message-only; a mid-chain remint id was never handed to the manager and would be
    ...
    const staleDirectiveProjection = (
      ...
      // doc). `redirect_worker` only stamps a correlatable id (`detail.queuedMsgId`, card 02621025) on the
      // HELD path (`delivered:false`); an IMMEDIATELY-delivered redirect (`delivered:true`, worker was idle)
      // never persists one (see deliverRedirect's own doc — "present only on the held path"), so there is no
      // durable record to walk a give-up/park chain against for that shape.
      const rootMsgId = (
        directiveEvent.kind === "message_worker" ? directiveEvent.detail?.msgId : directiveEvent.detail?.queuedMsgId
      ) as string | undefined;
      if (!rootMsgId) {
        // Two different reasons land here, both handled the same way (nothing further to walk):
        //   - a malformed message_worker record missing its own msgId (defensive — production always mints
        //     one; mirrors the pre-widening guard this replaces).
        //   - an immediately-delivered redirect_worker (see the doc just above): unlike message_worker's
        //     immediate path (which stamps BOTH `msgId` and `turnSeqAtDelivery` on its own event),
        //     redirect_worker's immediate path stamps NEITHER — so an immediate redirect that later silently
        //     gives up (the same async-confirmation risk worker_message's own tool docs describe) is not
        //     auditable from event data alone today. This is NOT for lack of an id to stamp: `deliverRedirect`
        //     (sessions/service.ts) already has `r.msgId` in scope on every path (`RedirectResult.msgId` is
        //     non-optional — `enqueueDurableMessage` mints and returns it unconditionally) and its own
        //     `queuedMsgId: r.delivered ? undefined : r.msgId` ternary is what deliberately discards it here —
        //     stamping it would be a one-line change, not new plumbing. CANDIDATE reason checked and RULED
        //     OUT: `queuedMsgId` has exactly one other consumer today, `staleQueuedMessageReason`'s
        //     self-match-exclusion guard (service.ts ~3298-3310, card 02621025) — it excludes a HELD
        //     redirect's own paired `session_message_queued` record from counting as "superseded by itself".
        //     But `enqueueDurableMessage` only ever appends that paired record `if (!r.delivered)`, so an
        //     IMMEDIATE redirect never has one to begin with — nothing there for an unconditionally-stamped
        //     id to collide with. No other reason for the omission is established here; whether to stamp it
        //     is a real open question, tracked separately (out of THIS card's scope, which only widens the
        //     existing HELD-path `queuedMsgId` into this projection's walk).
        //     Reported as `state:"delivered"` (it plainly did just go out as a turn) with `msgId:null` (there
        //     is nothing to correlate TODAY) rather than silently falling back to `"none"` or an older
        //     directive — see worker_redirect's own tool description for this same caveat stated to the
        //     reader.
      ...
      if (outcome.state === "parked") {
        ...
        // It clears the only way it honestly can: a NEWER message_worker OR worker_redirect becomes the
        // tracked directive (see the "latest wins across kinds" scan above) and this chain is never walked
        // again.
        return {
    ...
    server.registerTool(
      "worker_redirect",
      {
        description:
          ...
          "`directive`/`staleDirective`/`parkedDirective` projection DOES track a HELD redirect (delivered:false " +
          "here) the same way it tracks a held worker_message — including surfacing `parkedDirective` if Loom " +
          "gives up redelivering it entirely, which is exactly the case to watch for after a `delivered:false` " +
          "return from this tool. ⚠️ ONE REMAINING GAP: an IMMEDIATELY-delivered redirect (delivered:true here, " +
          "the worker was idle) carries no correlatable id for the projection to track further — it reads as a " +
          "plain `directive:{state:\"delivered\", msgId:null}` with no staleness tracking, so if THAT specific " +
          "hand-off silently gives up later (the same async-confirmation risk described above), nothing here will " +
          "flag it; fall back to checking the worker's own transcript/state if you have reason to doubt an " +
          "immediately-delivered redirect actually landed. Whichever kind (message or redirect) was sent most " +
          "recently is the ONE directive the projection tracks — a redirect always supersedes an outstanding " +
          "queued message (this tool flushes it), and a later worker_message likewise supersedes an outstanding " +
          "(even parked) redirect. " +
    ...
    server.registerTool(
      "platform_escalate",
      {
        description:
          ...
          "fresh Lead nudge, since the severity change is genuinely new information. Use it for " +
