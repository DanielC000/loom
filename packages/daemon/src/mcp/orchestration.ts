        ...
        try {
          ...
          if (!r.settled) {
            return ok({
              ...
              note: "batch still running — poll gate_status(opId) (read-only, never starts a new run) or gate_queue for a merge row with taskId:null/branch:null. Re-calling merge_batch with the SAME workerSessionIds is also safe at any delay: as long as the resolved candidate set is unchanged, it re-attaches to this SAME in-flight (or just-settled) op instead of starting a second one.",
            });
          }
          if (!r.ok) return ok({ error: r.error instanceof Error ? r.error.message : String(r.error) });
          return ok(r.value);
