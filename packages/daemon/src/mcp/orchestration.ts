 ...
 * (`reason`/`priorIdentity`/`currentIdentity`), not response-shaped. These two helpers turn it into the
 * `worker_merge_confirm` response fields + a reason-specific `note`, so a caller can never mistake an
 * announced fresh gate run for a replayed cached verdict. OBSERVED FIELDS ONLY — this reports what the
 * registry recorded (the identities), never asserts WHY they differ (e.g. never claims "main advanced";
 * `identity-mismatch` names the OBSERVATION, the note states the two shas and lets the caller judge).
 */
function freshMintResponseFields(freshMint: FreshMintInfo | undefined): Record<string, unknown> {
  if (!freshMint) return {};
  return {
    supersededBy: freshMint.reason,
    ...(freshMint.priorIdentity !== undefined ? { cachedVerdictIdentity: freshMint.priorIdentity } : {}),
    ...(freshMint.currentIdentity !== undefined ? { currentIdentity: freshMint.currentIdentity } : {}),
  };
}

/** DoD-5 (card 615967c5): the `untilSupersededVerdicts` cache is PROCESS-LOCAL — a daemon restart clears
 *  it (see PendingOpRegistry's own doc) — so "genuinely-new" must never read as "this branch has never
 *  been checked"; it may simply mean the cache was cleared. Neither other branch claims any durability
 *  property, so only that one needs the caveat. */
function freshMintNote(freshMint: FreshMintInfo): string {
  switch (freshMint.reason) {
    ...
    case "forced":
      return "forceRemoveWorktree:true bypassed any cached verdict" +
        (freshMint.priorIdentity !== undefined ? ` (which described commit ${freshMint.priorIdentity})` : "") +
        " and is running a genuinely fresh gate, as requested — not a cache replay.";
    case "genuinely-new":
      return "no cached verdict exists for this worker in this daemon process yet (this cache is process-local — " +
        "a daemon restart also produces this, even for a branch checked before) — this is a genuinely new gate " +
        "run, not a cache replay.";
  }
}

/**
 * Card 4aedde84 — the cache-hit-legibility fix: the mirror of {@link freshMintResponseFields} for the
 * branch that fix left silent. `r.cacheHit` (set by `PendingOpRegistry` ONLY on an actual cache-served
 * settle — see `CacheHitInfo`'s own doc) is registry-shaped, not response-shaped; this turns it into the
 * `servedFromCache` field a caller can check directly instead of having to notice the ABSENCE of
 * `supersededBy` — the exact incident this card fixes (a manager who had `supersededBy` in view on a
 * PREVIOUS call did not think to check for its absence on the next). `cacheHit` and `freshMint` are set in
 * disjoint branches of the SAME decision inside `attach()` (see that class's own doc), so the two markers
 * can never both appear — never derived from two independent checks that could theoretically disagree.
 */
function cacheHitResponseFields(cacheHit: CacheHitInfo | undefined): Record<string, unknown> {
  if (!cacheHit) return {};
  return { servedFromCache: true };
}

/** Card 4aedde84 — the note accompanying `servedFromCache`, naming what the incident showed a manager
 *  actually needed to notice: the opId being replayed (`opId` is `undefined` only for the rare thrown-error
 *  cache hit, whose raw error carries no opId of its own), the identity that verdict was validated against
 *  (when the registry knows it), and the ONE path that re-runs an unchanged commit. */
function cacheHitNote(cacheHit: CacheHitInfo, opId: string | undefined): string {
  return "served from cache: this is the SAME verdict already returned" +
    (opId !== undefined ? ` for op ${opId}` : "") +
    (cacheHit.identity !== undefined ? `, validated against commit ${cacheHit.identity}` : "") +
    " — no gate ran on this call, and the branch has not changed since that verdict was produced. " +
    "To force a genuine re-run of this exact, unchanged commit, re-call with forceRemoveWorktree:true " +
...
/**
 * Card 4491bd3b (DoD-1) — the `staleReport` threshold, the MIRROR of `STALE_DIRECTIVE_TURN_THRESHOLD`
 * for the opposite direction: a worker_report sitting `awaitingReview:true` while the MANAGER's own
 * turnSeq has advanced this many times since the report was recorded (`managerTurnSeqAtReport`, stamped
 * by `workerReport` in sessions/service.ts). Same turn-count-not-wall-clock rationale as the sibling
 * constant — a manager legitimately deep in one long turn right after the report lands reads as
 * turnsSinceReport ≤ 1, never a false alarm. This does NOT claim the report was lost, ignored, or seen —
 * only that the manager's session kept cycling turns while this specific report stayed unresolved, which
 * is the mechanism-agnostic, server-side-derivable signal the card asks for.
 */
...
function registerGateStatus(server: McpServer, sessions: SessionService, db: Db, scopeSessionId?: string, getScopeProjectId?: () => string | undefined, getRedactCrossProjectCallerProjectId?: () => string | undefined): void {
  ...
  const description = forWorker
    ? "Read-only status for YOUR OWN gate run, by the `opId` a `run_gate` {status:\"pending\"} " +
      ...
      "a passing run used to retain none of this at all). `gateDetail.failingTest` is a FLOOR, not a " +
      "complete account, unless `gateDetail.failingTestCount` reads exactly `1` — `undefined` count means " +
      "`failingTest` is itself `undefined` (nothing recognizable at all); `1` means the named test IS the " +
      "whole story; `>1` means it's only the LAST of several matching lines, and the others (possibly a " +
      "different file, possibly a different assertion shape) are NOT named anywhere in this result. Even at " +
      "`1`, this is a prompt to go run the file and enumerate — never proof by itself that nothing else " +
      "could be wrong. Never size a fix from `failingTest` alone when `failingTestCount` is `>1`; go read " +
      "the actual file. `proximity` ({nearBudget, step, fraction}, card " +
    ...
    : "Read-only status for ONE merge-gate run, by the `opId` a `worker_merge_confirm` " +
      ...
      "where `reason` itself is redacted. " +
      ...
      "diagnosis the `[loom:merge-rejected]` nudge embeds). `gateDetail.failingTest` is a FLOOR, not a " +
      "complete account, unless `gateDetail.failingTestCount` reads exactly `1` — `undefined` count means " +
      "`failingTest` is itself `undefined`; `1` means the named test IS the whole story; `>1` means it's " +
      "only the LAST of several matching lines, and the others (possibly a different file, possibly a " +
      "different assertion shape) are NOT named anywhere in this result. Even at `1`, this is a prompt to " +
      "go run the file and enumerate — never proof by itself that nothing else could be wrong. Never size a " +
      "fix from `failingTest` alone when `failingTestCount` is `>1`; go read the actual file. " +
      "`gateCap`/`concurrentGates`/`concurrentGatesMax` (card e2b6f900) are the gate CONCURRENCY this op " +
      "ran under — present on BOTH a pass and a fail via THIS tool (before this card a PASSING merge " +
      "carried none of this HERE — neither this tool's return nor the durable store behind it — and a " +
      "FAILING one only baked it as unstructured TEXT into the `[loom:merge-rejected]` nudge, never here. " +
      "`gate_history`, a separate audit-event series, already had this triple on both outcomes before this " +
      "card; use it for a multi-row trend, this field for a single opId's own recovery). `gateCap` is the " +
      "resolved concurrency cap in force at admission. `concurrentGates` " +
      "is a SNAPSHOT AT ADMISSION ONLY — how many gates were admitted together the instant THIS one started " +
      "— and UNDERSTATES contention: a second gate joining seconds later never moves it. `concurrentGatesMax` " +
      ...
      "recorded\" (of either cause) is exactly the ambiguity `emitCompareNotApplicableKind` (card fd0d34da) " +
      "narrows: set IFF a real gate spawned AND the predicate itself returned `notApplicable:true` (never " +
      "on a genuine `true`/`false`, and never when no gate spawned at all) — one of `\"repo-out-of-domain\"` " +
      "(a REPO-LEVEL fact, checked via a dedicated `git ls-tree` against the repo's own tree, never inferred " +
      "from this diff alone: NONE of the four scope directories exist anywhere in this repo, so the " +
      "predicate could never decide ANY diff on it — every project that isn't Loom's own daemon package), " +
      "`\"path-out-of-scope\"` (covers BOTH remaining shapes, which share the identical actionable fact: " +
      "the predicate applies to this repo fine, just not — fully, or at all — to THIS diff. Either THIS diff " +
      "touched an out-of-scope path while the SAME diff also touches an in-scope one elsewhere, OR the " +
      "repo's own tree DOES have one of the four scope directories even though this particular diff doesn't " +
      "touch it), " +
      "`\"harness-config-unavailable\"`, `\"typescript-unresolvable\"`, `\"git-operation-failed\"`, " +
      "`\"empty-diff\"`, or `\"unparseable-diff\"`. `emitCompareIdenticalCount` (the " +
      ...
      "(a plain gate-fail rejection, a plain successful merge) — populated for \"merge\" rows only. " +
      ...
      "discriminate. Card 344ce950's (single-file) / 67030bb9's (bounded multi-file, AND — since 67030bb9 — " +
      "a batch op via merge_batch, not just a solo worker_merge_confirm) merge gate can retry UP TO " +
      "`MULTI_FILE_RETRY_MAX` identifiable failing test files together in isolation " +
      "before rejecting; if that retry passes, the merge lands as an ordinary `outcome:\"pass\"` — the SAME " +
      "shape a clean first-attempt pass has, unless you read these three fields. `retriedFile` is a " +
      "comma-joined list of the retried file name(s) (a bare single name for the N=1 case, byte-identical " +
      "to before card 67030bb9), or a literal `null` — DELIBERATELY a MEASURED NEGATIVE here, NOT this " +
      ...
      "runs THIS daemon's own `packages/daemon/scripts/test-daemon.mjs` hermetic suite — `identifyRetriableTestFiles` " +
      "hardcodes that layout, so on every other project `retriedFile` reads `null` by construction, not by " +
      "coincidence. ⚠️ `retryDeclineReason` (card 67030bb9) is NOT a field on THIS tool's return — a genuine " +
  ...
  server.registerTool(
    "gate_queue",
    {
      description:
        ...
        "project's OWN worker doctrine, whose DoD DEFAULTS every worker to running its affected test " +
        "file(s) directly rather than through `run_gate` (see CLAUDE.md's targeted-test default), so the " +
        "fleet's ORDINARY, EXPECTED behavior is semaphore-invisible host load, up to one lane per live " +
        "worker; and (2) ALL non-agent load — a game client, a browser, media — which measured as the " +
        "SINGLE BIGGEST layer (~88% sustained) on a real desktop host, invisible to this tool AND to any " +
        "worktree-scoped process check. If you actually need to know whether the BOX (not the queue) is " +
        "quiet, that is a DIFFERENT question this tool cannot answer — reach for the two narrower " +
        "instruments built for it instead, and keep them separate: `packages/daemon/scripts/" +
        "host-quiet-check.mjs` (a whole-box CPU load reading — the one that actually licenses the word " +
        "'quiet') and `host-attribution-check.mjs` (read-only, worktree-scoped process ATTRIBUTION only — " +
        ...
        "holder OR by ANY same-repo `merge` currently QUEUED, whichever arrived first (card eb491463: a " +
        "waiting merge blocks every not-yet-admitted same-repo worker so worker self-checks cannot starve " +
        "it) — TWO workers on the " +
...
export class OrchestrationMcpRouter {
  ...
  private myContext(sessionId: string): Record<string, unknown> {
    ...
    // ⚠️ Card c148f118: `composerDirtyLen` is the CONSERVATIVE reading only — it never assumes a
    // defensive clear-prefix actually landed, so ALONE it cannot distinguish "a clear failed" from "a
    // clear worked but the fresh write after it hasn't confirmed yet." `composerDirtyLenBelieved` is the
    // OPTIMISTIC counterpart (same getter shape, same null-vs-0 discipline) — read them TOGETHER: equal
    // means nothing to doubt, `Believed < composerDirtyLen` means a clear is unresolved and the gap is
    // exactly how many characters are in doubt. See getComposerDirtyLen/getComposerDirtyLenBelieved's own
    // docs (pty/host.ts) for the full mechanics.
    const composerDirtyLen = this.pty?.getComposerDirtyLen(sessionId) ?? null;
    const composerDirtyLenBelieved = this.pty?.getComposerDirtyLenBelieved(sessionId) ?? null;
    ...
    server.registerTool(
      "my_context",
      {
        description:
          ...
          "clean, NOT unknown) — a non-zero value here, with no turn progressing, MAY mean your own last " +
          "message never actually landed; one Enter in your terminal (or worker_flush, if you're a " +
          "worker) recovers a genuinely stranded one. `composerDirtyLenBelieved` (card c148f118, same " +
          "null-vs-0 discipline) is the OPTIMISTIC counterpart — `composerDirtyLen` never assumes a " +
          "defensive clear-prefix actually landed, `composerDirtyLenBelieved` always does. Read them " +
          "TOGETHER: equal means nothing to doubt; `composerDirtyLenBelieved` LOWER than `composerDirtyLen` " +
          "means a clear was attempted and is unresolved, and the gap is exactly how many characters are " +
          "in doubt. ⚠️ NOT decisive alone (card d4b3fa6c): a GIVE-UP " +
          "SUPPRESSED mark — Loom believed your turn already started because the engine produced output " +
          "after its final Enter attempt — has NO automatic clear path on its OWN generation and can " +
          "read non-zero indefinitely even though nothing is actually stranded (confirmed in production " +
          "against a verified-empty composer); it clears only once a LATER, unrelated delivery's own " +
          "confirm resets it. Pressing Enter (or worker_flush) is itself the free disambiguator — a " +
          "composer-empty-shaped result means nothing was actually stranded. " +
  ...
  private buildServer(sessionId: string, role: SessionRole, attributions?: Map<string, ToolAttributionResult>): McpServer {
      ...
      server.registerTool(
        "run_gate",
        {
          description:
            ...
            "pins LOOM_GATE_TEST_CONCURRENCY=3 on the spawned child itself (matching the merge gate's own default " +
            ...
            "`failingTestCount` (card 8fb09f4c) is how many lines matched that SAME winning tier — `undefined` " +
            "iff `failingTest` is itself `undefined`; `1` means `failingTest` is a complete account of the " +
            "failure; `>1` means it's only the LAST of several matching lines and the rest go unnamed here. " +
            "NEVER size a fix from `failingTest` alone when `failingTestCount` reads `>1` — go run the file " +
            "and enumerate; even at `1`, treat it as a prompt to confirm, not proof nothing else could be wrong. " +
    ...
    //
    // ⚠️ ON A `busy:true` ROW THIS IS ~NOW BY CONSTRUCTION, THEREFORE UNINFORMATIVE THERE — see
    // `PtyHost.getLastOutputAt`'s own doc (pty/host.ts) for the measured busy-vs-idle proof. Its actual
    // discriminating power lives entirely in the IDLE case (a STALE value on a row that isn't busy).
    //
    ...
    // incident that motivated this card was a mismatch (an `[prompt-mismatch-unmatched-remainder]`
    // specimen — see `Live.lastMismatchUnmatched`'s own doc in pty/host.ts for what that broader
    // "unmatchable" population covers) that Loom classified correctly, in real time, but had NO worker_list/
    // worker_status projection for — `lastMismatchReplay`/`lastMismatchFusion` were both (correctly) null,
    // and a reader concluded "not detected" from "not a replay, not a fusion". Adding a THIRD stored field
    // for the unmatched class would only move the gap one class over — the same blind spot reappears for
    // whatever fourth class gets added next. This is DERIVED instead — computed at read time from the SAME
    // three getters already read below, never stored on `Live` itself — so "did any mismatch fire recently"
    // is answerable from ONE field without knowing every class name in advance, and covering a future
    // fourth named field only ever needs one line added to `candidates` here, not a new field threaded
    // through every worker_list/worker_status row shape.
    const deriveLastMismatch = (workerId: string): { kind: "replay" | "fusion" | "unmatched"; gen: number; at: number } | null => {
      if (!pty) return null;
      const replay = pty.getLastMismatchReplay(workerId);
      const fusion = pty.getLastMismatchFusion(workerId);
      const unmatched = pty.getLastMismatchUnmatched(workerId);
      const candidates: Array<{ kind: "replay" | "fusion" | "unmatched"; gen: number; at: number }> = [];
      if (replay) candidates.push({ kind: "replay", gen: replay.gen, at: replay.detectedAt });
      if (fusion) candidates.push({ kind: "fusion", gen: fusion.gen, at: fusion.detectedAt });
      if (unmatched) candidates.push({ kind: "unmatched", gen: unmatched.gen, at: unmatched.detectedAt });
    ...
    const deriveUnresolvedCascade = (
      parkedDirective: { msgId: string; parkedAt: string } | null,
      mismatch: { kind: "replay" | "fusion" | "unmatched"; gen: number; at: number } | null,
    ): { msgId: string; parkedAt: string; mismatchKind: "replay" | "fusion" | "unmatched"; mismatchAt: number; note: string } | null => {
      if (!parkedDirective || !mismatch) return null;
      const parkedAtMs = new Date(parkedDirective.parkedAt).getTime();
      if (Math.abs(mismatch.at - parkedAtMs) > UNRESOLVED_CASCADE_WINDOW_MS) return null;
      return {
        msgId: parkedDirective.msgId,
        parkedAt: parkedDirective.parkedAt,
        mismatchKind: mismatch.kind,
        mismatchAt: mismatch.at,
        // DoD-3: the disambiguation pointer travels WITH the flag, not in a separate doc a reader has to
        // already know to go find. DoD-4: never a refusal/block, never steers toward worker_stop/recycle —
        // bias toward waiting, name the one read that actually settles it.
        note: "UNRESOLVED — could not be determined whether this directive was lost or landed: Loom's own " +
          "redelivery budget for this message exhausted (parked, no individual CONFIRMED) with a " +
          "[prompt-mismatch] logged on this worker in the same window. This is NOT proof of either reading " +
          "— do not guess, and do not worker_stop/recycle on this alone. The one check that actually " +
          "settles it: read this worker's own transcript (worker_transcript) for a turn that acted on the " +
          "parked directive's content.",
      };
    };

    // Card e55371c1 — the path-keyed aliasing signal. `gen`/`recycledFrom` track RECYCLE CHAINS ONLY and
    // give NO evidence about worktree-path sharing (see db.listSessionsAtWorktreePath's own doc for the
    // mechanism); this is the real-property replacement. `null` when THIS row has no worktreePath of its
    // own yet (no worktree was ever created for it) — NOT DETERMINABLE, never a silent measured zero.
    // `0` means genuinely measured: this row's own worktreePath IS set and no other session row shares it.
    ...
    // Card 5a8a74e1: whether `ctxInputTokens`/`ctxTurns`/`model` are EVER measurable for this worker's
    // harness — reuses the exact `HarnessCapabilities.contextTelemetry` name/flag codex-adapter.ts's own
    // header note (card a1916267) already rules gates all three (same hook-gated capture chokepoint), so a
    // manager can branch on "structurally unmeasurable on this harness" vs "not yet measured" (see
    // `neverCompletedTurn`) instead of inferring it from `harness` + prose.
    ...
    // Card c8304714 fallout: guards the METHOD, not just `pty` itself — `pty?.` alone only protects
    // against `pty` being null/undefined, it does NOT protect against a present-but-PARTIAL pty stub
    ...
    // Card d8eaa381 (batch-gate finding): same METHOD guard as `lastFlushAttribution` above, for the
    // same reason — these two are new getters a real `PtyHost` always has, but a hermetic `pty` STUB
    // (several test files wire one implementing only the getters their own scenario needs) may not.
    const pendingQueueDepth = (workerId: string): number | null =>
      typeof pty?.getPendingQueueDepth === "function" ? (pty.getPendingQueueDepth(workerId) ?? null) : null;
    const currentTurnBusyForMs = (workerId: string): number | null =>
      typeof pty?.getCurrentTurnBusyForMs === "function" ? (pty.getCurrentTurnBusyForMs(workerId) ?? null) : null;

    // Card bba13405: same METHOD guard as `pendingQueueDepth`/`lastFlushAttribution` above — a real
    // `PtyHost` always has `getCodexBootStuck`, a hermetic stub built before this card may not.
    const codexBootStuck = (workerId: string): { at: number; timeoutMs: number; unmet: string[] } | null =>
      typeof pty?.getCodexBootStuck === "function" ? (pty.getCodexBootStuck(workerId) ?? null) : null;
    ...
    const fleetView = async () => {
      const workers = db.listWorkers(managerSessionId).map((w) => {
        ...
        return {
          ...
          // Card 808ee811: `ctxInputTokens` pre-divided by this worker's own model window — the SAME
          // null-vs-measured discipline (null exactly when ctxInputTokens is null, never a measured 0),
          // so a manager reads the recycle-relevant number directly instead of computing the ratio by
          // hand from ctxInputTokens+model. Null structurally for a codex-harness worker too (no
          // hook-based capture on that harness — see codex-adapter.ts's own doc).
          ctxPct: contextPercentFor(w.ctxInputTokens, w.model),
          // Card 5a8a74e1 — see contextTelemetryFor's own doc. `false` here is what makes a codex worker's
          // `ctxInputTokens`/`ctxPct`/`ctxTurns`/`model` null reads decidable as "never measurable on this
          // harness" instead of the ambiguous "not yet measured" a bare null (or `neverCompletedTurn`,
          // which is about TURN COUNT, not harness capability) would otherwise suggest.
          contextTelemetry: contextTelemetryFor(w.harness),
          ...
          // Card d8eaa381: see pty.getPendingQueueDepth/getCurrentTurnBusyForMs's own doc comments for
          // the null-vs-measured discipline. `pendingQueueDepth` is AGNOSTIC (both harnesses);
          // `currentTurnBusyForMs` is CLAUDE-ONLY and reads null for a codex row, same convention as
          // `lastEngineOutputAt` (card a1916267).
          pendingQueueDepth: pendingQueueDepth(w.id),
          currentTurnBusyForMs: currentTurnBusyForMs(w.id),
          // Card bba13405: CODEX-ONLY (see pty.getCodexBootStuck's own doc) — non-null exactly while a
          // codex session's boot-readiness ceiling has fired and not yet been resolved by a late latch;
          // always null for a claude row, a codex row still within its boot window, or one not live here.
          codexBootStuck: codexBootStuck(w.id),
          ...
          lastPasteTripwireGiveUp: pty?.getLastPasteTripwireGiveUp(w.id) ?? null,
          lastFlushAttribution: lastFlushAttribution(w.id),
          pendingMerge,
          rateLimitedUntil: w.rateLimitedUntil ?? null,
          rateLimitDeadline: w.rateLimitDeadline ?? null,
          worktreePathAliases: worktreePathAliases(w.worktreePath, w.id),
          ...reportedProjection(w.id, pendingMerge),
          ...directiveProjection,
          ...archivedWithoutReport(w.id),
          // Card f797affb — see deriveUnresolvedCascade's own doc above.
          unresolvedCascade: deriveUnresolvedCascade(directiveProjection.parkedDirective, mismatch),
        };
      });
      const pendingSpawns = sessions.listPendingSpawns(managerSessionId).map((op) => ({
        ...
        ctxInputTokens: null,
        ctxPct: null,
        contextTelemetry: null, // no worktree/pty exists yet — harness not determinable here, never a silent false
        neverCompletedTurn: true,
        model: null,
        lastActivity: op.startedAt,
        lastEngineOutputAt: null,
        composerDirtyLen: null,
        composerDirtyLenBelieved: null,
        unconfirmedDeliveryMs: null,
        pendingQueueDepth: null,
        currentTurnBusyForMs: null,
        codexBootStuck: null, // no worktree/pty exists yet — not applicable
        lastMismatchReplay: null,
        lastMismatchFusion: null,
        lastMismatch: null,
        lastMismatchNoticeSuppressed: null,
        lastPasteTripwireGiveUp: null,
        lastFlushAttribution: null,
        pendingMerge: null,
      ...
      const archivedUnreported = db.listWorkerSessionIdsWithEventKind(["worker_exited_without_report"])
        ...
        .map((w) => ({
          ...
          pendingQueueDepth: null,
          currentTurnBusyForMs: null,
          codexBootStuck: null, // not live in this process
          lastMismatchReplay: null,
          lastMismatchFusion: null,
          lastMismatch: null,
          lastMismatchNoticeSuppressed: null,
      ...
      const dangling = (await sessions.getDanglingWorkers(managerSessionId)).map((d) => ({
        ...
        ctxInputTokens: null,
        ctxPct: null,
        contextTelemetry: null, // harness isn't tracked on a DanglingWorkerEntry (already stopped) — not determinable here
        ...
        pendingQueueDepth: null,
        currentTurnBusyForMs: null,
        codexBootStuck: null, // already stopped, not live in this process
        lastMismatchReplay: null,
        lastMismatchFusion: null,
        lastMismatch: null,
        lastMismatchNoticeSuppressed: null,
      ...
      // gap: an entirely EMPTY fleet (zero rows of any kind) has no row to carry it — covered instead by
      // composeManagerStartupPrompt's boot-time orchestration block and by worker_spawn's own `capacity`
      // on the FIRST spawn, neither of which needs an existing row.
      // `?.` defends the same way the `pty?.` reads above do — several hermetic tests in this file's own
      // test suite wire a plain stub object (not a real SessionService) as `sessions`, and a stub that
      // predates this field would otherwise throw at runtime despite type-checking fine under `as any`.
      const capacity = sessions.getWorkerCapacity?.(managerSessionId) ?? { cap: 0, live: 0, inFlight: 0, free: 0 };
      return [...workers, ...pendingSpawns, ...capQueued, ...archivedUnreported, ...dangling].map((r) => ({ ...r, capacity }));
    };

    server.registerTool(
      "worker_list",
      { description: "List the workers you (this manager) have spawned — your direct children. Card 548a0c7e: EVERY row (real worker or placeholder alike) also carries `capacity:{cap,live,inFlight,free}` — the SAME live worker-concurrency snapshot a successful worker_spawn's own `capacity` field carries (see that tool's description for what each sub-field means), repeated identically on every row rather than hoisted to a wrapper (this tool deliberately stays a bare array — see worker-list-pending-ops.mjs's own pin against a `{workers,...}` wrapper). Read it off ANY row when you have one; an entirely EMPTY fleet (no rows at all) has none to read it from — but that case is still directly derivable, not a second hand-transcription: an EMPTY array means the fleet is EMPTY, so `live` is 0 and `free == cap`, and `cap` is the value named in your startup prompt's Orchestration config block. `directive` (`{msgId, state, at}`, state one of `\"none\"|\"pending\"|\"delivered\"|\"parked\"|\"confirmed-after-park\"`) is your MOST RECENT worker_message OR worker_redirect's raw resolved state (whichever you sent most recently), always keyed to the msgId YOU got back — `\"none\"` means you've never messaged or redirected this worker; `\"pending\"` means it's still queued or Loom is still retrying its redelivery internally, not yet resolved either way; `\"delivered\"`/`\"parked\"` are explained by `staleDirective`/`parkedDirective` below, which are derived ALARM flags layered on top of this same state (read those for the \"is something wrong\" signal; read `directive` when you need the raw state, e.g. to tell a fresh `\"pending\"` apart from a worker you've simply never messaged). `staleDirective` (non-null: `{msgId, deliveredAt, turnsSinceDelivery}`) flags a worker_message OR worker_redirect directive that was DELIVERED but has no worker_report since — distinct from the `{delivered:true}` worker_message/worker_redirect itself returns, which only proves the text was HANDED OFF as a turn attempt (or durably queued), never that the engine confirmed it or that it was acted on — see worker_message's own docs for why `delivered:true` is not a delivery guarantee. Fires only once the worker has completed several of its OWN real turns with no report at all since delivery (never on a single long-running turn, however long); clears the instant any worker_report lands, or once you send a newer worker_message OR worker_redirect (tracking always follows your MOST RECENT directive of EITHER kind — a `worker_redirect` is tracked the same way a `worker_message` is, INCLUDING going stale/parked, on BOTH its held and immediately-delivered paths; see worker_redirect's own docs). `parkedDirective` (non-null: `{msgId, parkedAt}`) is the THIRD state alongside `staleDirective` — a directive Loom gave up REDELIVERING entirely: your worker_message's (or a worker_redirect's, on either path — held or immediately-delivered, see worker_redirect's own docs) hand-off never confirmed, Loom retried it internally (in-session, then across several re-mint attempts) and exhausted its budget, so it PARKED — a PARK is NOT proof the directive was never received: the engine can confirm a write minutes late under load, so the original may still land on its own; `directive.state` MAY still resolve to `\"confirmed-after-park\"` (below) if a late confirming hook proves that, but that resolution is not guaranteed even when the directive genuinely did land, so its absence is NOT evidence the directive failed (card 085d9422, relocated here from the `[loom:redelivery-parked]` notice, which used to spell this hedge out per-event) — this is the case `staleDirective` alone cannot see (a parked directive never reaches the worker, so the worker's own turnSeq never advances and `staleDirective` would otherwise read `null` forever, indistinguishable from \"nothing wrong\"). Mutually exclusive with `staleDirective` (a directive is either delivered-but-unacknowledged, or never-actually-delivered). ⚠️ STICKY, unlike `staleDirective`: a `worker_report` does NOT clear it — the text never reached the worker, so nothing the worker reports can be an acknowledgement of it. It clears the same way `directive` itself moves on — once you send a NEWER worker_message OR worker_redirect — OR on its own, the instant a LATE confirming hook proves the original hand-off actually ran (Loom's own retry budget can give up minutes before the engine does — see memory `engine-confirmation-can-lag-minutes-timeouts-assume-seconds`): that shows up as `directive.state:\"confirmed-after-park\"` with `parkedDirective` back to `null` — a DIFFERENT, decidable resolution from a still-genuinely-parked directive, checkable by simply re-calling this tool later, no notice required. ⛔ On a directive that is STILL `parkedDirective` (not `confirmed-after-park`), RE-SEND — Loom will not retry it for you; but if you see `directive.state:\"confirmed-after-park\"`, do NOT resend — the original already landed and a resend would create a real duplicate turn. `reportedState` (done|blocked|null) + `awaitingReview` flag a worker that has called worker_report and is sitting idle awaiting your review (cleared once you worker_message/worker_redirect it, it's recycled or stopped, or its merge actually COMPLETES — merely reviewing its diff with worker_merge, or a rejected/cancelled merge, does NOT clear this; the worker is still awaiting your decision). `staleReport` (card 4491bd3b; non-null: `{reportedAt, managerTurnsSinceReport}`) is the RECONCILIATION DETECTOR for the reverse direction from `staleDirective` — it fires ONLY while `awaitingReview` is true, once YOUR OWN session has completed several further turns since this report was recorded with THIS SAME report still unresolved. It does NOT claim the report was lost, misrendered, or evicted — only that your session kept turning while it sat there unconsumed by any of your own review/merge/message actions; if you see this and don't recall the report, pull `worker_report_get(workerSessionId)` on this worker — the report is durably recorded server-side, read straight from that record (not a rendered transcript), and recoverable in full with NO exceptions (unlike `worker_transcript`, it does not depend on the worker's CURRENT engine transcript file — see card 8a5bd0d0, engine session id rotation, below — or on transcript rendering never having dropped anything, see card 68d85015). `worker_transcript(lastN:2)` still works for the overwhelmingly common case where this worker's engine session never rotated, but has a standing exception `worker_report_get` does not share: a report transcribed to a pre-rotation transcript file is NOT concatenated into `worker_transcript`'s read of the current file and is not recoverable that way — see `worker_transcript`'s own description for the caveat; Loom keeps no per-session signal telling you whether this worker ever hit it. Clears the same way `awaitingReview` itself does (you worker_message/worker_redirect it, it's recycled/stopped, or its merge completes). `archivedWithoutReport:true` flags a worker whose pty EXITED WITHOUT EVER CALLING worker_report — it no longer counts against your concurrency cap, but it did NOT silently vanish: this row is a distinct ADDITIVE category (a real worker row, `processState` reflects its actual exit state) that appears ONLY for a genuinely-unreported strand, never for a worker that reported (even `noChanges`) and cleanly auto-retired — pull `worker_transcript` to see what it did, then `worker_merge` to review/merge any committed work, or re-dispatch the task. It clears on its own once you resolve it (move the task off its active lane, or spawn/recycle a successor). `pendingMerge` (non-null) on a row means a worker_merge_confirm for it is either still in flight (`state:\"running\"`) OR has JUST settled (`state:\"done\"|\"failed\"` with an `outcome` of `\"merged\"|\"rejected\"|\"cancelled\"|\"unknown\"|\"stale-base\"|\"worktree-dirty\"` (card 479f449f — \"unknown\" replaces the old \"failed\": a THROWN exception mid-confirm can never prove the merge genuinely failed, only that this confirm could not determine the outcome either way; check the `[loom:merge-unknown]` nudge, or `gate_status(opId)`'s own `outcome:\"error\"` verdict, before assuming nothing landed. Card 99a1cf6f — \"stale-base\" means the gate genuinely ran (and would have squashed) but canonical MAIN advanced during it, a BENIGN race distinct from an ordinary \"rejected\" (a real test failure): re-confirming now genuinely re-gates instead of replaying a cached refusal, so treat it as \"try again, nothing wrong with the branch,\" not a code problem to investigate — see `worker_merge_confirm`'s own description for the full rejection wording) — a brief, COSMETIC display of the terminal result, visible here for only a few seconds after settling; it reverting to `null` does NOT mean the verdict is forgotten, see below). ⚠️ `state:\"running\"` means an op is IN FLIGHT for this worker — it is set the INSTANT the merge is minted, which can be well before the gate is ever submitted for admission, so it does NOT by itself mean the gate is EXECUTING; don't read a long-`running` pendingMerge as evidence of a wedge on its own. While `state:\"running\"`, `gatePhase` disambiguates WAITING from EXECUTING without a second call: `\"queued\"` means admitted-pending (waiting on the daemon-global gate cap or a same-repo sibling — see `gate_queue`'s own docs on why a queued entry's elapsed time is WAIT time, never run time), `\"running\"` means the gate is actually admitted and executing, and `null` means the op hasn't reached gate admission yet (worktree prep/union-merge still in progress) or has already left the live gate registry (gate step finished, squash/finalize still running) — `null` is a normal reading, not an error. Once `state` is no longer `\"running\"` the merge is ALREADY DONE — read `outcome` here directly, no need to re-call worker_merge_confirm just to learn it. Re-calling worker_merge_confirm for a settled merge is ALWAYS SAFE, at ANY delay — seconds, minutes, or hours, long after this row has reverted to `null` — because the verdict is cached separately from this display window: at the SAME commit it returns that SAME cached verdict; if the branch has picked up new commits since, it gates them for real instead — a different question, never a silent re-run of the SAME question. The ONLY way to force a genuine RE-RUN of the SAME commit (e.g. retrying a flake, or completing a nested-repo worktree cleanup) is `forceRemoveWorktree:true` on the re-call — name it explicitly: it is the deliberate retry escalation, not merely a worktree-cleanup option, and it always runs for real. `rateLimitedUntil`/`rateLimitDeadline` (non-null) mean the worker is PARKED ON A USAGE CAP — busy will read false but this is NOT a healthy idle worker; it resumes on its own once `rateLimitedUntil` passes (`rateLimitDeadline` is the give-up horizon). `lastEngineOutputAt` is an INTRA-TURN liveness signal, distinct from `lastActivity` (which only moves at turn boundaries): it advances on every chunk of engine output, so it keeps moving THROUGH a single long turn — a recent `lastEngineOutputAt` on a `busy:true` row means the engine is alive and emitting; a stale one means it may be wedged. ⚠️ ON A `busy:true` ROW, \"RECENT\" IS ~NOW BY CONSTRUCTION AND THEREFORE UNINFORMATIVE THERE — a live engine emits output every few seconds regardless of whether the turn is converging, so this is a tautology on a busy row, not itself a health signal; its actual discriminating power lives entirely in the IDLE case, a STALE value on a row that ISN'T busy (Platform Lead verification, card 0f15b692: five genuinely busy rows all read the same read-instant timestamp, while two idle rows sat minutes stale). LIVENESS is not CONVERGENCE: a retry loop, or a worker re-reading the same file, emits just as steadily as real work, so recent output alone never proves progress. `lastActivity` advancing (a turn boundary crossed) is the strongest signal this row carries toward that — a wedged turn cannot complete — though a loop spanning several turns still crosses boundaries normally, so it only clears the turn that just ended. For actual progress, order your checks by WHICH CASE EACH ONE CAN ACTUALLY ANSWER, not by cost — a cheap check that silently fails on the hard case is worse than no check, because it still hands back an answer. Lead with a `worker_transcript` pull and look for a NAMED, distinct artifact — output, a file, a result that could not exist unless the new work actually ran; the same shape repeated with only an index/timestamp changed is not progress. This is the ONLY one of the three checks here that still works on a worker that has never completed a turn — precisely the class where \"is it progressing?\" gets asked most urgently (card 8d1abb7a). `ctxInputTokens` climbing across two reads and `git log`/`git status` on its branch are real signals once a worker is past that point, but BOTH return FALSE NEGATIVES on a never-completed-turn worker, and neither says so on its face: `ctxInputTokens` reads `null` until a turn actually completes (see `neverCompletedTurn` below — that null is a decidable, EXPECTED state on such a worker, never a sign of trouble), and a healthy worker deep in a long first turn legitimately shows a byte-identical `git status`/`--numstat` across samples minutes apart, because nothing has landed yet — flat means \"no commit yet,\" never \"stalled.\" Check `neverCompletedTurn` first: while it's `true`, only the transcript-artifact check can discriminate; once it's `false`, all three are meaningful and the cheaper two are fine to lead with. `neverCompletedTurn` (`true` when this worker's `turnSeq` is still 0 — it has not finished a single turn yet) is what makes `ctxInputTokens: null` decidable instead of a bare null a reader might mistake for a broken measurement: `true` means \"expected, no data yet, absence is not bad news\"; `false` alongside a still-`null` `ctxInputTokens` would itself be the anomalous case worth investigating, not the routine one. ⚠️ **`turnSeq` counts LOOM-TURNS (one Loom submit → confirmed-idle cycle), never a harness's own internal transcript-record count** (card 5a8a74e1, closing a real pilot misreading confirmed structurally via the daemon's own event log: zero `message_worker`/`flush_worker_composer` events for the worker in question, against a 3,600-event positive control DB-wide — so a SECOND Loom-turn genuinely never happened). On codex, ONE Loom-turn routinely renders as MANY transcript records — even codex's own first-use `AGENTS.md`+environment-context injection lands as its own additional user-role record ahead of Loom's actual kickoff, so counting \"turns\" by eyeballing role-tagged records (even counting only user-role ones) overcounts and is not a substitute for `turnSeq`. ⛔ NEVER COMPARE `turnSeq` AGAINST `ctxTurns` — different units, not two readings of the same thing: `ctxTurns` counts assistant lines including every intra-turn tool round trip, `turnSeq` counts only completed turns, so a large gap between them (e.g. ctxTurns:1535 vs turnSeq:529) is the NORMAL case for one long turn, never a signal on its own (Platform Lead verification, card 0f15b692, filed after that exact comparison drove a p1 retraction on card `6198b93e`). `turnSeq` survives a daemon restart intact — it is not reset to 0 on resume. `null` on the `\"dangling\"` placeholder row means not tracked (that worker is already stopped, not mid-turn). Never read `ctxInputTokens: null` as a measured `0` — it isn't occupancy, it's absence of a measurement. `ctxPct` (card 808ee811) is `ctxInputTokens` pre-divided by this worker's own model context window (`contextWindowForModel`), rounded to a whole percent — the SAME null-vs-measured discipline: null EXACTLY when `ctxInputTokens` is null (never a measured 0), including structurally for a codex-harness worker (no hook-based capture on that harness — see codex-adapter.ts's own doc). Read it at the moment you're deciding continue-vs-recycle instead of computing the ratio yourself from `ctxInputTokens`+`model`. `contextTelemetry` (card 5a8a74e1, boolean|null) is the field that makes a `null` `ctxInputTokens`/`ctxPct`/`ctxTurns`/`model` DECIDABLE by CAUSE rather than just by TIMING: it mirrors the adapter's own `HarnessCapabilities.contextTelemetry` flag for this worker's harness — `false` means these fields are STRUCTURALLY unmeasurable on this harness (never a hook-relay chokepoint to capture them, today's codex reality — see codex-adapter.ts's own doc), so a persistent null there is expected and not a sign of trouble; `true` (claude today) means the null is only \"not yet measured\" (pair with `neverCompletedTurn` for that read). `null` means not determinable from this placeholder row (no worktree/pty exists yet, or harness isn't tracked on this row shape) — never read as a measured `false`. `composerDirtyLen` is a count of characters possibly still sitting UNSUBMITTED in this worker's composer from an earlier delivery that never confirmed (null when not live in this process; 0 means genuinely clean, NOT unknown — never read an absent signal as a measured zero) — SET synchronously the moment a give-up/heal fires, so it stays non-zero and readable even when nothing further ever arrives (the exact stuck case this exists to catch); CLEARED either by a confirming hook decisively proving THIS generation's own mark actually landed, or by a SUBSEQUENT, unrelated delivery attempt's own defensive clear-prefix going on to confirm. ⚠️ CONSERVATIVE ONLY (card c148f118): this never assumes a defensive clear-prefix actually landed, so \"a clear was attempted and failed\" and \"a clear worked but the write after it just hasn't confirmed yet\" would read as the exact same number ALONE — read it together with `composerDirtyLenBelieved` (same row, same null-vs-0 discipline): equal means nothing to doubt, `composerDirtyLenBelieved` lower than this means a clear is unresolved and the gap is exactly how many characters are in doubt. ⚠️ NOT AUTHORITATIVE ALONE (card d4b3fa6c): a GIVE-UP SUPPRESSED mark specifically — the engine produced output after Loom's own final Enter attempt, so Loom believes the turn already started — has NO automatic clear path on its own generation (confirmed twice in production: a stale non-zero read against a VERIFIED-EMPTY composer, once idle post-turn, once busy mid-first-turn) and clears ONLY via that later, unrelated delivery. Do NOT treat a non-zero value, even paired with a stalled `lastActivity`/`lastEngineOutputAt`, as proof this worker's last message never reached it — call `worker_flush` FIRST (submit-only, writes nothing, a documented no-op on a genuinely clean composer) to disambiguate before trusting the read or reaching for a destructive remedy (`worker_recycle`/`worker_stop`); see memory `engine-confirmation-can-lag-minutes-timeouts-assume-seconds` for the fuller failure picture and recovery guidance. `null` means the session isn't live in this daemon process. `lastMismatchReplay` (card 68459420; non-null: `{gen, replayedGen, reportedLen, intendedLen, detectedAt}`) is the SENDER-directed arm of this worker's own `[loom:prompt-mismatch]` notice: it fires the instant that notice identifies what the engine actually submitted as a byte-for-byte REPLAY of an earlier generation this worker itself wrote (`replayedGen`) instead of the NEW content Loom wrote for `gen`. Card b7158b99 — CORRECTION: this is a POSSIBLE loss, not an established one, and the worker cannot verify it either way: it only ever sees what arrived, never what was intended for it. The composer may still hold `gen`'s own intended text, and a LATER generation's own submission can fuse it back in whole — a subsequent `lastMismatchFusion` on this SAME worker naming `gen` in its own `spanGens` means it was recovered, not lost (measured live: a manager's own gen=9 replay was fully recovered by gen=10's confirmed fusion, spanGens=[8,9,10]). Whether recovery happens is unknowable until that later generation, if any, actually occurs. Read it here, at the point you already look, rather than relying solely on the worker's own session-facing notice text — but do NOT re-send solely because this is non-null: check whether a LATER `lastMismatchFusion` on this worker already names `gen` (recovered — do not re-send) before concluding a re-send is warranted. STICKY, like `parkedDirective`: never cleared once set (so it survives even if you don't happen to look right away), and OVERWRITTEN (not accumulated) by a later occurrence — always reflects the LATEST replay only, so an old `detectedAt` you've already acted on is safe to ignore. `null` means either no such replay has been detected yet, or the session isn't live in this process (the two are not distinguished, mirroring `unconfirmedDeliveryMs`'s own null-collapsing precedent below — no manager decision turns on telling them apart). `lastMismatchFusion` (card f5f6515a; non-null: `{gen, spanGens, reportedLen, intendedLen, detectedAt}`) is the FUSED counterpart to `lastMismatchReplay` above, but its CONTRACT IS THE OPPOSITE — read this one carefully, it is not a restatement of the other: it fires when the engine-reported content is instead a CONFIRMED CONCATENATION of more than one generation this worker itself wrote (the composer accumulated instead of clearing — `spanGens` names every generation involved, oldest-first, no cap on how many). Because that span is always a contiguous run ending with the CURRENT generation's own write, the content Loom intended for THIS turn ALWAYS arrived — nothing was lost, so there is nothing to re-send. The risk instead is DUPLICATION: an EARLIER generation named in `spanGens` (besides the last) may have its own turn already run, so a manager reading a non-null `lastMismatchFusion` should check whether that earlier directive's own effects already happened before treating anything as unconsumed — never re-send it. STICKY / overwritten-not-accumulated, same as `lastMismatchReplay`; a given mismatch sets at most ONE of the two fields, so read both. `lastMismatchNoticeSuppressed` (card c0323f8a; non-null: `{gen, writtenHash, reportedHash, count, detectedAt}`) is a THIRD, DIFFERENT signal from the two above — it does NOT mean a mismatch was detected; it means this worker's own session-facing `[loom:prompt-mismatch]` notice was about to be delivered a SECOND time, byte-identical (same gen, same writtenHash, same reportedHash as the notice already sent), and Loom suppressed the resend rather than deliver a duplicate turn. `count` accumulates across repeats of the SAME signature (reset to 1 the moment a genuinely different signature suppresses, which cannot happen without an intervening real notice first) — read it to tell `suppressed once` from `suppressed repeatedly`. This is a DEFENSIVE guard against a re-mint path that is code-reachable but has, as of this field's own introduction, no confirmed observed instance — do not read a non-null value here as proof the underlying trigger is understood, only as proof the guard itself fired. STICKY / overwritten-not-accumulated as a struct (only `count` accumulates), same posture as `lastMismatchReplay`/`lastMismatchFusion`. `null` means no suppression has fired yet this session, or the session isn't live in this process (not distinguished, same convention as the two siblings above). `lastPasteTripwireGiveUp` (card 72cab648; non-null: `{gen, token, engineSessionId, detectedAt}`) is a DIFFERENT failure class from the three `lastMismatch*` fields above — those track a REPLAY/FUSION/unmatched substitution of composer content on an ordinary turn; this fires when the bare-paste-placeholder tripwire's own ONE-SHOT auto-recovery re-injection ALSO collapsed, and Loom will not retry a third time (see card eef4883c/0f9268cc — the recovery mechanism itself, unrelated to detection). It is the SENDER pull-surface for that give-up, additive alongside the existing `console.warn` and the session+sender `enqueueSystemNudge` (card 47c11741) — read this at the point you already look, rather than relying solely on the notice reaching you (pinned memory `shipping-a-detector-is-not-someone-reading-it` measured that attention-path notice alone at zero acted-on). Same PULL-surface mechanics as `lastMismatchReplay`/`lastMismatchFusion`: never cleared once set, overwritten (not accumulated) by a later give-up, `null` = none fired yet this session, session-not-live is not distinguished from that (same convention as its siblings). `lastFlushAttribution` (card ac7884e3; non-null: `{gen, attributable, reason, resolvedAt}`) is the FLEET-SCAN read of the SAME sticky verdict `worker_flush` itself returns — see that tool's own description for the full three-way reading (`null`/`attributable:true`/`attributable:false, reason:\"marker-superseded-before-confirm\"`) and `pty/host.ts`'s `Live.lastFlushAttribution` doc for the underlying mechanics; this field is not restated here. Read it here when you want to know whether an EARLIER `worker_flush` on this worker eventually resolved, without re-calling `worker_flush` (which is read-only ONLY when the composer happens to already be clean — a non-empty composer makes it a real, state-changing flush, not a pure read). STICKY, same convention as its siblings above. `lastMismatch` (card 31f3d047; non-null: `{kind, gen, at}`, `kind` one of `\"replay\"|\"fusion\"|\"unmatched\"`) is a GENERIC, DERIVED view over `lastMismatchReplay`/`lastMismatchFusion`/`lastMismatchUnmatched` — computed at read time as whichever of the three has the most recent `detectedAt` (re-tagged as `gen`/`at`), NOT a fourth stored field: the incident this card responds to was a mismatch Loom classified correctly (as neither a replay nor a fusion) with no worker_list/worker_status projection at all, and a stored fourth field would only relocate the same gap to whatever fifth class comes next. `\"unmatched\"` covers `lastMismatchUnmatched`'s own broader population — any mismatch that matched none of the recognized/confirmed shapes (a plain unrecognized substitution, or a partial substring-remainder recognition — see that field's own doc in pty/host.ts) — so \"did any mismatch fire recently, and roughly what kind\" is answerable from this ONE field, without knowing every class name in advance or checking three separately-named fields. `null` means none of the three has fired yet since this session went live, or the session isn't live in this process (not distinguished, same convention as its siblings) — a mismatch that reconciled as benign (whitespace re-render, a stale placeholder prefix, a chunk-seam form feed) never sets any of the three underlying fields either, by design, so `null` here is also the correct reading for those. `unresolvedCascade` (card f797affb; non-null: `{msgId, parkedAt, mismatchKind, mismatchAt, note}`) flags the ONE combination `5c3db367`'s forensic close proved costs real archaeology to rule out: `parkedDirective` non-null (a give-up/re-mint cascade PARKED with no individual CONFIRMED) CO-OCCURRING with `lastMismatch` non-null (a `[prompt-mismatch]` logged on this worker) within a bounded window around the park. It is pure WIRING over the two fields above — no new detection, nothing stored — combined because BOTH readings ('silently lost' and 'succeeded anyway, just never individually confirmed') are live possibilities for this identical signature, proven both ways in the specimen that motivated this field. ⛔ It NEVER guesses which: `note` says plainly this is UNRESOLVED, not a verdict, and names the ONE check that actually settles it — read this worker's own transcript (`worker_transcript`) for a turn that acted on the content — rather than relocating the archaeology by raising an alarm with nothing to act on. It is NOT a refusal, a block, or an auto-remedy, and it does NOT mean stop or recycle this worker — treat it exactly like a plain `parkedDirective` otherwise (re-send only if you've ruled out that it already landed) with one extra fact: a mismatch ALSO fired nearby, which is what makes the 'already landed' reading as live as the 'lost' one here. Clears the same way its two inputs do (a newer directive supersedes `parkedDirective`, or the mismatch/park pair ages out of the correlation window on a later read) — never sticky beyond that. `unconfirmedDeliveryMs` (card a33a72f7) is an EARLIER signal than `composerDirtyLen`: milliseconds since this worker's CURRENT generation's Enter was written, for as long as it stays unconfirmed by a hook — non-null the INSTANT a write is outstanding, with no dependency on `composerDirtyLen`'s own give-up/heal budget (30s-plus) ever firing; `null` means nothing is currently outstanding (never submitted, or already confirmed) or the session isn't live. It does NOT distinguish 'still within Loom's own retry budget' from 'Loom already gave up on this exact generation, outcome still unknown' — a give-up never resets it. Read it together with `composerDirtyLen`: zero there while this is non-null means genuinely in-flight and never given up (the one case this adds unambiguous new information); non-zero there is ambiguous either way (may be this generation's own give-up, or stale residue from an earlier superseded one still awaiting its confirm-driven clear). `pendingQueueDepth` (card d8eaa381) is how many messages currently sit in this worker's FIFO send queue right now — EVERY entry regardless of sender (your own direction, the worker's own re-queued text, a Loom-authored nudge), not just messages you sent; `null` when the session isn't live in this process, `0` is a genuine measured empty queue (AGNOSTIC — reads for both claude and codex workers, since the queue mechanism itself is harness-independent). Only ONE entry drains per completed turn, so a non-zero depth means your own most recent direction may take several more of the worker's own turns to actually land — it does NOT mean delivery is stuck; cross-check `currentTurnBusyForMs` and `lastEngineOutputAt` before concluding anything is wedged. `currentTurnBusyForMs` (card d8eaa381) is milliseconds elapsed since this worker's CURRENT turn went busy, or `null` if it's idle right now (nothing in flight) or the session isn't live; CLAUDE-ONLY — always `null` for a codex worker (no equivalent concept exists on that harness yet, same convention as `lastEngineOutputAt`). A LARGE `currentTurnBusyForMs` on a `busy:true` row is NOT by itself evidence of a wedge: a real turn with heavy tool/subagent use can legitimately run tens of minutes (this card's own diagnosis measured a genuine, healthy 53-minute single turn) — a manager that can't tell that apart from a dead worker is the exact risk this field and `pendingQueueDepth` exist to close; pair both with `lastEngineOutputAt` (recent = still emitting) rather than reading either alone. `codexBootStuck` (card bba13405; non-null: `{at, timeoutMs, unmet}`, `unmet` a subset of `[\"ready marker\", \"model-loaded\", \"trust-dialog-resolved\"]`) is CODEX-ONLY and closes the gap that motivated it: a resumed/spawned codex session wedged before booting reads `processState:\"live\"`, `resumability:\"resumable\"`, `busy:false` — EXACTLY what a healthy idle worker shows, with `pendingQueueDepth` staying silent too on a quiet worker — so none of those fields discriminate it. This one does: it goes non-null the instant codex's own boot-readiness ceiling fires without ever latching (the same fail-loud `[loom:codex-boot-stuck]` moment your session already gets nudged about, now durably readable here too, not just at the instant it fired). `at` is when the ceiling fired, `timeoutMs` the ceiling itself, `unmet` names which condition(s) were never simultaneously observed. Treat non-null the same way the notice already recommends — manager intervention (inspect the session, `worker_message`/`worker_redirect`/`worker_stop`+respawn) if it stays quiet, since it may still self-recover if codex was merely slow. ⚠️ `null` here does NOT mean healthy on its own — it's also the reading for a claude row (never applicable), a codex row still within its own boot window (hasn't had time to time out yet), or one not live in this process; read it alongside `processState`/`busy`/`pendingQueueDepth` as usual. STICKY only until genuine resolution: a LATE marker sighting can still latch boot readiness normally (no retry ladder — see the ceiling's own doc), and doing so clears this back to `null`, so a session that goes on to recover reads healthy again rather than staying permanently flagged. A worker_spawn still running past the sync-wait budget shows up as an ADDITIVE placeholder row: `workerSessionId:null`, `pendingSpawn:{opId,startedAt}`, `processState:\"starting\"` — not a real worker yet, so don't count it as live or awaiting review; poll here or re-call worker_spawn (same taskId/agentId/kickoffPrompt) to fetch the result. A worker_spawn REJECTED outright because the concurrency cap was full ALSO shows up as an ADDITIVE placeholder row — distinct from the pending one above: `workerSessionId:null`, `processState:\"cap-queued\"`, `capQueued:{opId,agentId,taskId,kickoffLabel,queuedAt}` — the intent never started at all. WHILE THIS DAEMON PROCESS KEEPS RUNNING, it AUTO-FIRES on its own (FIFO — oldest queued first) the next time a concurrency slot on this manager actually frees, with no re-call needed; you can still re-`worker_spawn` the same taskId/agentId by hand if you don't want to wait, which clears the marker the same way it always did. ⚠️ IT DOES NOT SURVIVE A DAEMON RESTART OF ANY KIND (a crash, a dev-watch reload, or a deliberate `daemon_restart`) — the queue is in-memory only, by design, and a restart discards every entry silently with no error and no trace here; a graceful `daemon_restart` (whoever triggers it) notifies EVERY affected manager/platform what was dropped on the other side, not just the one who called it, but a crash or dev-watch restart gives no such notice at all — re-`worker_spawn` by hand after ANY restart if a dispatch you made before it still matters. If an auto-fire attempt itself fails for a real reason (its task went terminal/held, its worktree couldn't be created, …) you get a `[loom:cap-queue-autofire-failed]` message naming the opId/task/agent — the entry is dropped at that point and needs a fresh worker_spawn. To withdraw a queued entry BEFORE it fires, call `worker_stop({opId})` (see worker_stop's own docs) — it never auto-dispatches after that. A STOPPED (archived) worker whose branch/worktree still holds unmerged work (card ba41b402) ALSO shows up as an ADDITIVE placeholder row: `processState:\"dangling\"`, `worktreePath` set (a field no other row carries) — this is a REAL past worker (`workerSessionId` is its actual id, not null) that is ALREADY ARCHIVED and holds NO concurrency-cap slot (the cap only ever counts `processState===\"live\"` rows against your `maxConcurrentWorkers`; an archived row is excluded before that filter even runs, and stays excluded for as long as it shows up here — card 5ab3c664 measured this directly: a dangling row never once blocked or delayed a fresh worker_spawn), surfaced because its bound task has no recorded merge yet (or, for a taskless worker, its branch carries at least one commit not on the mainline) — content-based, not branch-existence-based, so a branch whose work already shipped under a different name/fix never appears here. It is purely observational (nothing here deletes or merges anything); pull `worker_transcript`/`git log` on its branch to see what it holds, then `worker_merge_confirm` or re-dispatch as you would any other unmerged work. `worktreePathAliases` (card e55371c1) is the PATH-KEYED worktree-aliasing signal — the count of OTHER session rows (any process state, any manager on this project) whose worktree_path is EXACTLY this row's, measured fresh on every read. ⛔ `gen`/`recycledFrom` are NOT an aliasing check and never were — they track RECYCLE CHAINS ONLY, and `branch`/`worktreePath` are both a pure function of `taskId` (see `taskKey`, git/worktrees.ts), so a plain re-spawn after a rejected merge lands a SECOND `gen:0`/`recycledFrom:null` row on the SAME worktree path with no recycle anywhere in sight — a clean `gen`/`recycledFrom` reading provides no evidence at all about this. Same null-vs-0 discipline as `composerDirtyLen`: `0` is a MEASURED zero (this row's own worktreePath IS set, and genuinely nothing else shares it); `null` means NOT DETERMINABLE HERE — this row has no worktreePath of its own yet (the `\"starting\"`/`\"cap-queued\"` placeholders, before any worktree is created), never a silent zero. A worktreePath is always `WORKTREES_DIR/<projectId>/[<repoKey>/]<taskKey>` — the `projectId` segment means two sessions in DIFFERENT projects can never share one, so this per-project count is already the complete answer to \"is this aliased anywhere\", not a narrowed one; there is no separate platform-wide aliasing question left to measure. Deliberately a COUNT, never the aliasing rows' own ids — a reader here may not be authorized to see a sibling manager's worker identity, so this never widens what a foreign row reveals, only that aliasing exists. A `0` is a snapshot, not a guarantee: a later re-spawn onto the same path can raise it at any time. Never count any of the three placeholders (`\"starting\"`, `\"cap-queued\"`, `\"dangling\"`) as live or awaiting review.", inputSchema: strictShape({}) },
    ...
    server.registerTool(
      "worker_status",
      {
        description: "Get the full session record for one of your workers, by workerSessionId. Includes the derived `reportedState` (done|blocked|null) + `awaitingReview` flag — set when the worker has called worker_report and is idle awaiting your review, cleared once you worker_message/worker_redirect it, it's recycled or stopped, or its merge actually COMPLETES (merely reviewing its diff with worker_merge, or a rejected/cancelled merge, does NOT clear this). Also includes `staleReport` (see worker_list) — non-null once your OWN session has completed several further turns since this report was recorded with it still unresolved; asserts NO cause, only that your session kept turning while this specific report sat unconsumed — pull `worker_report_get(workerSessionId)` to recover it in full, straight from durable event storage — with none of `worker_transcript(lastN:2)`'s caveats (a rotated engine session id, card 8a5bd0d0; a rendered/truncated transcript, card 68d85015). Also includes `ctxPct` (see worker_list) — `ctxInputTokens` pre-divided by this worker's own model context window, rounded to a whole percent; null exactly when `ctxInputTokens` is null, never a measured 0. Also includes `contextTelemetry` (see worker_list; card 5a8a74e1) — boolean|null, the field that tells a structurally-unmeasurable-on-this-harness null (codex today) apart from a not-yet-measured one (pair with `neverCompletedTurn`); null means not determinable here. Also includes `neverCompletedTurn` (see worker_list) — `true` while this worker's `turnSeq` is still 0 (it has not finished a single turn yet), which makes a `null` `ctxInputTokens` decidable (\"no data yet\", not a broken measurement) instead of a bare, ambiguous null. ⚠️ `turnSeq` counts LOOM-TURNS (submit → confirmed-idle), never a harness's own internal transcript-record count — see worker_list's own `neverCompletedTurn` entry for the full reasoning and the codex-specific trap (a real pilot misreading, card 5a8a74e1). Also includes `archivedWithoutReport` (see worker_list) — true only when this worker's pty EXITED WITHOUT EVER CALLING worker_report and the strand is still unresolved; NEVER true for a worker that reported (even `noChanges`) and cleanly auto-retired. Also includes `directive` (see worker_list) — your most recent worker_message OR worker_redirect's raw resolved state (`\"none\"|\"pending\"|\"delivered\"|\"parked\"|\"confirmed-after-park\"`), whichever you sent most recently. Also includes `staleDirective` (see worker_list) — a delivered worker_message or worker_redirect with no worker_report since, once several of the worker's own real turns have passed with no report. Also includes `parkedDirective` (see worker_list) — a worker_message or HELD worker_redirect Loom gave up redelivering entirely (exhausted its internal retry/re-mint budget with no confirmed hand-off); mutually exclusive with `staleDirective`, STICKY (a worker_report does NOT clear it — only a newer worker_message or worker_redirect does — OR a late confirming hook resolving it to `directive.state:\"confirmed-after-park\"`, see worker_list's own docs for that third state), and the signal that catches a directive that never reached the worker at all — re-send it, UNLESS `directive.state` already reads `\"confirmed-after-park\"`, in which case the original landed and resending would duplicate it. (A worker_redirect is tracked this way on either delivery path — held or immediately-delivered; see worker_redirect's own docs.) Also includes `lastEngineOutputAt`, the intra-turn liveness signal (see worker_list) — recent means the engine is alive and emitting, stale means it may be wedged; neither proves the work is actually converging (see worker_list for why, and what to check instead). Also includes `composerDirtyLen` (see worker_list) — a count of characters possibly still sitting UNSUBMITTED in this worker's composer from an earlier delivery that never confirmed; null when not live in this process, 0 means genuinely clean (never read an absent signal as a measured zero). ⚠️ CONSERVATIVE ONLY (card c148f118, see worker_list for the full reading guide): never assumes a defensive clear-prefix actually landed. Also includes `composerDirtyLenBelieved` (same null-vs-0 discipline) — the OPTIMISTIC counterpart; read the two together — equal means nothing to doubt, `composerDirtyLenBelieved` lower means a clear is unresolved and the gap is how many characters are in doubt. Also includes `unconfirmedDeliveryMs` (see worker_list) — an EARLIER signal than `composerDirtyLen`: milliseconds since this worker's current generation's Enter was written, for as long as it stays unconfirmed by a hook; non-null the instant a write is outstanding, unlike `composerDirtyLen` which needs a give-up/heal to actually fire first. Does NOT by itself distinguish still-retrying from already-gave-up-outcome-unknown — cross-check `composerDirtyLen` (see worker_list for the full reading guide). Also includes `pendingQueueDepth` (see worker_list; card d8eaa381) — how many messages sit in this worker's FIFO send queue right now, every sender counted, not just yours; null when not live, 0 is a genuine measured empty queue. Only one entry drains per completed turn — a non-zero depth means your own direction may take several more turns to land, not that delivery is stuck. Also includes `currentTurnBusyForMs` (see worker_list; card d8eaa381) — milliseconds since this worker's current turn went busy, null if idle or not live, always null for a codex worker (no equivalent concept on that harness). A large value here on a busy worker is not by itself evidence of a wedge — pair it with `lastEngineOutputAt` before concluding anything is stuck. Also includes `codexBootStuck` (see worker_list; card bba13405) — CODEX-ONLY, non-null (`{at, timeoutMs, unmet}`) exactly while a codex session's boot-readiness ceiling has fired without ever latching, the discriminator for a wedged-at-boot codex session that otherwise reads identically to a healthy idle one on `processState`/`resumability`/`busy`; `null` for a claude row, a codex row still within its own boot window, or one not live here; cleared automatically if a late marker sighting still latches boot readiness normally. Also includes `lastMismatchReplay` (see worker_list) — non-null means this worker's own `[loom:prompt-mismatch]` notice identified a REPLAY of an earlier generation it wrote: a POSSIBLE loss of whatever you last sent it, NOT an established one (card b7158b99), which the worker cannot verify itself — a later `lastMismatchFusion` on this same worker naming this `gen` means it was recovered, not lost; check for that before re-sending. Also includes `lastMismatchFusion` (see worker_list) — the FUSED counterpart, with the OPPOSITE contract from `lastMismatchReplay`: non-null means the reported content was a CONFIRMED concatenation of more than one generation this worker wrote (`spanGens` names all of them) — the current turn's own content always arrived intact, so DO NOT re-send; check instead whether an earlier named generation's own directive already ran, to avoid acting on it twice. Also includes `lastMismatchNoticeSuppressed` (see worker_list) — non-null means a byte-identical repeat of this worker's own `[loom:prompt-mismatch]` notice was suppressed rather than delivered as a duplicate turn; `count` tells you how many times, for the current signature. A defensive guard, not evidence the underlying re-mint trigger is understood. Also includes `lastPasteTripwireGiveUp` (see worker_list) — the SENDER pull-surface for the bare-paste-placeholder tripwire's own give-up (its one-shot auto-recovery re-injection ALSO collapsed, and Loom will not retry a third time); a DIFFERENT failure class from `lastMismatchReplay`/`lastMismatchFusion`/`lastMismatch`, additive alongside the existing `console.warn` and session+sender nudge (card 47c11741), same PULL-surface mechanics (never cleared once set, overwritten by a later occurrence). Also includes `lastFlushAttribution` (see worker_list) — the sticky `worker_flush` attribution verdict, readable here without re-calling that tool. Also includes `lastMismatch` (see worker_list) — a GENERIC, DERIVED `{kind, gen, at}` view (`kind` one of `\"replay\"|\"fusion\"|\"unmatched\"`) over `lastMismatchReplay`/`lastMismatchFusion`/`lastMismatchUnmatched` above, computed at read time as whichever fired most recently (by `detectedAt`) — NOT a fourth stored field, so \"did any mismatch fire recently, and roughly what kind\" is answerable from this one field without knowing every class name in advance; `null` means none of the three has fired yet, or the session isn't live in this process. Also includes `unresolvedCascade` (see worker_list) — non-null (`{msgId, parkedAt, mismatchKind, mismatchAt, note}`) only when `parkedDirective` AND `lastMismatch` both fired within a bounded window of each other: the exact ambiguous cascade `5c3db367` proved costs a two-file forensic read to rule out. `note` states plainly this is UNRESOLVED (never a guessed verdict) and names the one check that settles it (read this worker's own transcript for a turn acting on the content) — it is NOT a refusal/block and does NOT mean stop or recycle this worker. Called with NO workerSessionId, it returns the fleet view (same as worker_list) so a reflexive no-arg call just works. OPTIONAL `msgId`: `directive`/`staleDirective`/`parkedDirective` above only ever track your MOST RECENT worker_message/worker_redirect to this worker — the instant you send a newer one, an OLDER directive's own resolution becomes invisible there even though it keeps resolving on its own. Pass the ROOT msgId a PRIOR worker_message/worker_redirect call returned to you (never a re-mint id — you were never handed one) to get `queriedDirective: {msgId, found, state, at}` for THAT SPECIFIC directive regardless of what you've sent since: `found:false` means this worker has no record of that root msgId at all; `found:true` gives its own `state` (`\"pending\"|\"delivered\"|\"parked\"|\"confirmed-after-park\"`) independent of the tracked `directive` field. This is the per-message consumed/not-consumed read — check a specific earlier directive before treating a `parked` notice about it as a reason to recycle or re-spawn, even after you've since sent something newer to the same worker. Also includes `worktreePathAliases` (see worker_list) — the path-keyed count of OTHER session rows sharing this row's exact worktree_path; `gen`/`recycledFrom` are NOT an aliasing check (they track recycle chains only), use this field instead. Same null-vs-0 discipline as `composerDirtyLen`: `0` is measured (nothing else shares this path), `null` means this row has no worktreePath of its own to key on.",
      ...
      async ({ workerSessionId, msgId }) => {
        ...
        return ok({
          ...
          // Card 808ee811 — see fleetView's identical field for the full doc.
          ctxPct: contextPercentFor(w.ctxInputTokens, w.model),
          // Card 5a8a74e1 — see contextTelemetryFor's own doc (fleetView) for the full reading guide.
          contextTelemetry: contextTelemetryFor(w.harness),
          neverCompletedTurn: (w.turnSeq ?? 0) === 0,
          lastEngineOutputAt: pty?.getLastOutputAt(w.id) ?? null,
          composerDirtyLen: pty?.getComposerDirtyLen(w.id) ?? null,
          composerDirtyLenBelieved: pty?.getComposerDirtyLenBelieved(w.id) ?? null,
          unconfirmedDeliveryMs: pty?.getPendingConfirmMs(w.id) ?? null,
          // Card d8eaa381 — see fleetView's identical fields for the full doc.
          pendingQueueDepth: pendingQueueDepth(w.id),
          currentTurnBusyForMs: currentTurnBusyForMs(w.id),
          // Card bba13405 — see fleetView's identical field for the full doc.
          codexBootStuck: codexBootStuck(w.id),
          lastMismatchReplay: pty?.getLastMismatchReplay(w.id) ?? null,
          lastMismatchFusion: pty?.getLastMismatchFusion(w.id) ?? null,
          lastMismatch: mismatch,
    ...
    server.registerTool(
      "worker_transcript",
      {
        description:
          ...
          "When that happens this tool — like every transcript reader — can only see the CURRENT " +
          "post-rotation file; content written only to the abandoned pre-rotation file is NOT concatenated " +
          "in and is not recoverable through this tool. This is a STANDING, ALWAYS-ON caveat, not a flag " +
          "that fires only on an affected read — Loom keeps no per-session record of whether a rotation " +
          "ever happened, so a normal transcript that never rotated looks identical to one that silently " +
          "did; a shorter-than-expected or seemingly-incomplete history is the only observable symptom.",
        inputSchema: strictShape({
    ...
    server.registerTool(
      ...
      {
        description:
          "Read one worker_report your worker already FILED, straight from durable event storage — " +
          "WITHOUT the worker needing to be alive, and without going through rendered transcript text at " +
          "all (card 68d85015). A worker_report's full body is recorded verbatim in an orchestration event " +
          "the INSTANT the worker calls worker_report — this reads that record directly, so it is unaffected " +
          "by anything that can degrade a transcript rendering (an oversized tool-call argument, a rotated " +
          "engine session id, a spilled/truncated page). THIS is the documented recovery path for a " +
          ...
          "YOUR OWN scratch dir instead, with `summary` replaced by {summaryFile, summaryChars, note}; a " +
          "summary at or under the cap is always inline, byte-identical to what was filed.",
        inputSchema: strictShape({ workerSessionId: z.string(), eventId: z.string().optional() }),
      },
      async ({ workerSessionId, eventId }) => {
        ...
        if (eventId) {
          ...
          if (r.kind === "ambiguous") return ok({ error: `ambiguous worker_report event id-prefix '${eventId}' — it matches ${r.ids.join(", ")}; pass more characters or the full id` });
          if (r.kind === "none") return ok({ error: `no worker_report event matching id ${eventId}` });
          target = r.record;
        } else {
        ...
        const spill = spillTextIfLarge(managerSessionId, "report-spills", target.id, summary, SPILL_INLINE_BUDGET_CHARS);
        return ok({
          eventId: target.id,
          ts: target.ts,
          ...
          ...(spill.inline
            ? { summary }
            : {
                summaryFile: spill.file,
                summaryChars: spill.chars,
                note:
                  `summary is ${spill.chars} chars — too large to inline safely, written to ${spill.file} as ` +
                  "plain UTF-8 text. Page it with Read or grep it for a keyword.",
              }),
        });
      },
    );
    ...
    server.registerTool(
      ...
      async ({ taskId, agentId, kickoffPrompt, kickoff, reviewOfWorkerSessionId, reviewOfTaskId }) => {
        ...
        try {
          ...
          // absent), so a normal spawn's result is byte-identical to before this field existed. A
          // re-call landing within the retain window is answered from cache: the WORKER already exists
          // and its kickoff was already delivered ONCE, at the original spawn — this call does NOT
          // re-deliver it, so a manager that wants to hand it something new must use worker_message.
          const reused = r.cacheHit
            ? { reused: true, reusedNote: "this worker already existed (re-call within the retain window) — its kickoff was NOT re-delivered; use worker_message for any new direction." }
            : {};
          ...
          const dirty = worker.reusedDirtyWorktree
            ...
            : {};
          // Board card 13cc2300: additive-only — omitted entirely when nothing was destroyed (fresh spawn,
          // reattached-branch-only worktree, reused-but-already-clean worktree, or a >0-ahead recovery
          // branch that was never recut). DISTINCT from `dirty` above and never merged into it: `dirty`
          // means "survived and is still dirty — go reconcile it"; this means the OPPOSITE — Loom's own
          // pre-spawn re-cut already discarded these tracked edits, so there is nothing left in the tree to
          // reconcile. The worker's OWN kickoff already carries this same fact (worker-prompt.ts's
          // `discardedBlock`); this is the manager-facing mirror so it's visible without reading the
          // worker's transcript.
          const discarded = worker.discardedOnRecut
            ? {
                discardedOnRecut: true,
                discardedOnRecutStatus: worker.discardedOnRecut,
                discardedOnRecutWarning: `heads up: this worktree was reused from a prior attempt, but its branch was 0 commits ahead of the mainline, so Loom re-cut it onto current main BEFORE this spawn — a \`git reset --hard\` that DISCARDED ${worker.discardedOnRecut.fileCount} prior tracked path(s) (see \`discardedOnRecutStatus\`). That prior work is gone, not merely uncommitted — factor it into your review; the worker's own kickoff already knows.`,
              }
            : {};
          ...
          // Card 961da6c6: additive-only — omitted entirely unless the fleet's codex default was SKIPPED for this worker
          // (its profile needs something codex cannot honour), so every other spawn's result is byte-identical.
          const harnessSkipped = worker.harnessDefaultSkipped && worker.harnessDefaultSkipped.length > 0
            ? { harnessDefaultSkipped: { items: worker.harnessDefaultSkipped, note: "the fleet default harness is codex, but this worker's profile needs something codex cannot honour, so it was spawned on claude instead — see items." } }
            : {};
          // Card 548a0c7e: live worker-concurrency snapshot ({cap, live, inFlight, free}) — the ONLY
          // caller-visible way this cap was previously observable was by provoking CapQueueRejectedError
          // (see that class's own message). This is the exact moment a manager decides whether to
          // dispatch again, so it's on the SUCCESS response, not just the rejection.
          return ok({ workerSessionId: worker.id, branch: worker.branch, worktreePath: worker.worktreePath, ...shipped, ...dirty, ...discarded, ...stale, ...review, ...harnessSkipped, ...reused, capacity: worker.capacity });
        } catch (e) {
    ...
    server.registerTool(
      ...
      async ({ workerSessionId, opId, mode }) => {
        ...
        if (opId) {
          const r = sessions.cancelCapQueuedSpawn(managerSessionId, opId);
          if (r.outcome === "ambiguous") {
            return ok({ error: `ambiguous opId prefix '${opId}' — it matches ${r.ids.join(", ")}; pass more characters or the full id` });
          }
          return ok(r.outcome === "cancelled" ? { cancelled: true } : { cancelled: false, reason: r.reason });
        }
        try {
    ...
    server.registerTool(
      "question_ask",
      {
        description:
          ...
          "— see question_pull's own doc for the resulting {state, detail} shape. \"credential\" — ask for a secret " +
    ...
    server.registerTool(
      "question_pull",
      {
        description:
          ...
          "never will be — keep orchestrating and check back later for that case instead.",
        inputSchema: strictShape({}),
      },
      async () => {
        ...
        // whichever question drained AS this turn (it already delivered — never queued), NOR any nudge
        // for a `remaining` row this pull deliberately left 'answered'.
        if (consumedIds.length > 0) {
          sessions.purgeAnsweredQuestionNudges(managerSessionId, consumedIds);
        }
    ...
    server.registerTool(
      "requests_list",
      {
        description:
          ...
          "returned, offset, hasMore}: `items` per row is {id, projectId, loomSessionId, filedBySessionId, " +
          "agentId, taskId, type, title, state, createdAt, answeredAt, consumedAt} plus an answer summary by " +
          "type — chosenOption/note for decision|input, approved/note/scope/expiresAt/lapsed/fulfillment for " +
          "permission (see question_pull's own doc for `fulfillment: {state, detail}` — an approval is an " +
          "authorization, not evidence the human-only write it authorizes actually happened; \"unknown\"/" +
          "\"unwritable\"/\"not_yet_done\"/\"fulfilled\", recomputed fresh on every read), ack ONLY for credential " +
          "(NEVER the secret — a pending row's answer fields read null rather than a misleading false-ish " +
    ...
    server.registerTool(
      "worker_merge",
      {
        description:
          ...
          "taskless worker (no card to preview a title from) — a taskless worker instead gets " +
          "`tasklessSubjectPreview` (card 7a1a76e9 DoD-4), the subject the eventual squash will actually " +
          "derive from THIS branch's own tip commit (never a fabricated placeholder, and never the branch " +
          "name — that's only the merge's own last-resort fallback for an unreadable branch). NECESSARY " +
          "but NOT SUFFICIENT on its own: this preview being populated does NOT mean the eventual QUEUED " +
          "merge will announce its landed subject anywhere else — see `worker_merge_confirm`'s own " +
          "description for where the landed subject is actually surfaced once this merge is confirmed. " +
          "BATCH-BOUND PREVIEW (card a32533a1): `commitSubject`/`tasklessSubjectPreview` above preview ONLY " +
          "the SOLO squash path (worker_merge_confirm) — a `merge_batch` landing never rewrites or coerces " +
          "anything, it commits each candidate branch's OWN commit subjects verbatim. The result ALSO " +
          "carries `ownTipSubject` (the branch's tip commit subject, UNCOERCED — byte-for-byte what a batch " +
          "would actually commit for this branch) and `ownTipSubjectConventional` (a plain " +
          "toConventionalSubject(ownTipSubject) === ownTipSubject fact, mirroring `coerced`'s own framing) — " +
          "computed for EVERY worker regardless of task, so you can title-check what a batch would land " +
          "before routing this worker into `merge_batch` instead of a solo confirm. Absent only when the " +
          "branch has no readable commit at all. NON-TIP COMMITS (card 591906ae): `ownTipSubject` above " +
          "covers only the branch's LAST commit — for a branch with MORE THAN ONE commit, `merge_batch` " +
          "lands every earlier commit verbatim too, and those are invisible to the tip fields alone. The " +
          "result ALSO carries `ownNonTipCommitSubjects` — every OTHER commit's own subject on this " +
          "branch (`<mergeBase>..<branch>`, oldest-first, excluding the tip), bounded (a `true` " +
          "`ownNonTipCommitSubjectsTruncated` means older entries were dropped, never silently) — plus " +
          "`ownNonTipCommitSubjectsConventional`, whether EVERY subject actually returned is Conventional-" +
          "Commits form (when truncated, this verdict covers only the visible entries, not the dropped " +
          "tail). Absent entirely for the common single-commit branch, where `ownTipSubject` alone is " +
          "already the complete check — no need to look further.",
    ...
    server.registerTool(
      "worker_merge_confirm",
      {
        ...
        description: "STEP 2: after reviewing, confirm the merge. Runs the build/DoD gate, and ONLY if green merges the branch as ONE squash commit, removes the worktree, and moves the task to done. The staged set is re-derived at confirm time (never a stale snapshot), so a valid +N-commit branch merges on the FIRST call. Fail-closed: a failed gate or a conflict leaves the repo untouched and the worktree retained. A genuine no-op is distinguishable via emptyKind: ALREADY_MERGED (branch already in main → finished idempotently, merged:true) vs STAGE_EMPTY_RETRY (no diff to merge → merged:false, worktree retained). A THIRD, distinct outcome is `cancelled:true` (card 361520a0): a manager's `gate_cancel` withdrew this confirm while it was still QUEUED, before the gate ever ran — `merged:false`, but this is NOT a rejection (no gate ran, nothing to diagnose, nothing held against the branch); `cancelKind` names how, and the async signal is `[loom:merge-cancelled]`, never `[loom:merge-failed]`. Re-call worker_merge_confirm when ready to retry for real. A gate rejection (reason:\"build gate failed\") carries `gateDetail: {phase, failedStep, failingTest, failingTestCount, stderrTail, exitCode, signal, timedOut, steps}` — the failing phase (typecheck|test|build) if derivable, the failed step's own command, the winning tier's own matching line if extractable (highest-priority tier wins when several are present; the live per-step scan keeps that tier's LAST occurrence, while the rarely-taken output-tail fallback — used only when the live scan found nothing and a tail exists — takes its FIRST instead, so don't assume either ordering without checking which producer supplied it; and never a line recording a PASS, from any tier, from EITHER producer: card 2f0b2e57 fixed a real incident where an unanchored tier matched a PASSING assertion whose own label happened to contain the tier's keyword). `failingTestCount` (card 8fb09f4c) is how many lines matched that SAME winning tier — `undefined` iff `failingTest` is itself `undefined` (no fabricated `0` or `1`); `1` means `failingTest` is a complete account; `>1` means it's only the LAST of several matching lines and the rest (possibly a different file, possibly a different assertion shape) go unnamed here — NEVER size a fix from `failingTest` alone when `failingTestCount` reads `>1`, and even at `1` treat it as a prompt to go run the file and confirm, not proof nothing else could be wrong. And the failing step's own output tail — CONTENT-SELECTED (card 6ffee3e2), not positional: bounded to ~4KB (`OUTPUT_TAIL_BYTES`) when it's the last step's plain trailing output, but up to ~16KB (`FAILURE_BLOCK_CAP_BYTES`) when it instead recovers a real per-file assertion body from a `test-daemon.mjs` `FAILURES:` block that would otherwise have been truncated off — so you can diagnose a real test failure vs. a flake vs. a build break without re-running the gate blind; the same detail is also folded into the `[loom:merge-rejected]` notification text. ADMISSION-TIME RE-UNION (card b798e706): the base this gate validates is re-derived the instant this merge is actually admitted off the shared gate queue, not frozen from before the wait — if canonical main moved during that wait, it re-unions against the CURRENT main tip and can still land on its first pass instead of always self-aborting with a stale-base refusal; a merge whose base never moved is unaffected, byte-identical to before this existed. HONEST SCOPE (Code Review correction): this closes the gap for main moving during the wait for reasons OTHER than a same-repo sibling's own squash — an out-of-band/REST commit, cross-project queueing noise, a sibling whose squash happened to land before this op's own admission — and is never worse than doing nothing. It does NOT reliably close the more common same-repo-sibling case: the per-repo admission guard (card 92e960d1) frees the instant a sibling's GATE finishes, before that sibling's OWN squash lands (which runs after, outside that guard) — so a merge admitted right behind a sibling typically re-derives against a main that sibling hasn't squashed onto YET, runs its own full gate, and can still self-abort with `gate_base_invalidated` once that sibling's squash lands mid-run. Closing that case needs holding the admission guard across the squash phase too — a larger, separate change, carded as `c24dd48a`. If the re-derivation ITSELF hits a real git conflict (main advanced in a way that no longer merges cleanly with this worktree's already-unioned tree), the confirm is REJECTED with reason `union_conflict_at_admission` (or `union_merge_failed_at_admission` for a non-conflict git failure, worded WITHOUT asserting main-advanced as the cause — that framing is only earned by an actual conflict) — squash phase never reached, canonical repo untouched, worktree retained; this is a real failure needing your attention (rebase/resolve, or investigate a possible transient git/filesystem issue for the non-conflict case), distinct from the benign, purely-informational `gate_base_invalidated` race described below, which needs only a plain re-confirm — and, since card 99a1cf6f, is a DELIBERATE EXCEPTION to the \"same commit gets the SAME cached verdict\" rule stated a few sentences down: because a stale-base rejection's validity depends on canonical MAIN, not the branch, this ONE outcome is never served from the cache regardless of how many times you re-confirm at the identical commit — a plain re-confirm after it genuinely re-gates for real, no `forceRemoveWorktree:true` needed (that flag is still the only way to force a re-run of an ORDINARY cached verdict — a real pass or a real test-failure rejection). DIRTY-WORKTREE REFUSAL (card 975c774b): the gate runs IN the worker's live worktree, so its verdict is only meaningful if that tree matches a commit. The worktree is stamped right before every gate spawn and again when it settles: a tree that is ALREADY dirty (uncommitted files) when the gate is about to spawn is refused up front with `reason` \"worktree has uncommitted changes when the gate was about to spawn\" (the gate never runs), and a tree whose stamp CHANGED across the run (an uncommitted edit, or a new commit, landed while the gate ran) has its PASS refused — nothing is squashed — with `gateWorktreeDirty:{phase:\"before-gate\"|\"during-gate\",detail}` and a `[loom:merge-rejected]` reason `gate_worktree_dirty`; a FAIL keeps its ordinary rejection with the same `gateWorktreeDirty` flag. Like `stale-base`, these outcomes (`\"worktree-dirty\"`) are NEVER cached: commit or discard the worker's uncommitted changes, then a plain re-confirm re-gates for real. HONEST SCOPE: an edit created AND removed entirely inside the gate window is not detectable by a start/settle stamp. A PASS is also refused when the BRANCH TIP moved after the gate spawned (a commit landed mid-gate, tree clean): nothing is squashed, `gateTipMoved:{phase:\"pre-squash\"|\"in-lock\",gated,live}` + `[loom:merge-rejected]` reason `gate_tip_moved`, never cached (outcome `\"gate-tip-moved\"`) — a plain re-confirm re-gates the new tip. GATE OUTPUT ON A PASS TOO (card a1a8c5c4): on the two DOMINANT outcomes — a plain gate-fail rejection, or a plain successful merge (not reused, not gateless, and not one of the rarer post-gate-pass rejections below) — the result ALSO carries a top-level `outputTail`, the SAME last-step tail described in the gate-rejection sentence above — ~4KB on a PASS (a passing step never needs content-selection, so it's always the plain `OUTPUT_TAIL_BYTES` trailing tail), and on a FAIL potentially up to the ~16KB content-selected bound instead — persisted durably (readable later via `gate_status(opId)` even after this call's own response is long gone). Before this card a PASSING merge gate's output was retained NOWHERE at all, closing off the one question every gate investigation eventually needs: whether some condition seen in a failing run ALSO occurred in a passing one. A rarer post-gate-pass rejection (a merge conflict, `gate_base_invalidated`, an orphaned/stage-empty no-op) still returns no `outputTail` even though a gate genuinely ran — its absence there is NOT proof no gate ran; `gateExtended` (present, possibly `false`, whenever a gate spawned) is what actually proves that. If the project has NO gateCommand configured, a successful merge carries {warning:\"unverified: ...\"} — the merge landed but was NOT checked by any build/DoD gate. INERT-DIFF SKIP (card db9b0130): a gate IS configured but was deliberately skipped because the branch's ENTIRE changed-path set is under `docs/` — provably inert (cannot change any test outcome) — carries `gateRan:false` (no `reusedOpId`) plus {warning:\"merge gate skipped: ...\"}, distinct from both a gateless project and a reused self-check; a diff touching even one path outside that allowlist always runs the full gate for real, fail-closed. A successful merge that created a NEW squash commit (i.e. not the ALREADY_MERGED no-op path) also echoes `commitSubject` — the exact subject it just committed — so a transcript reader can see what landed without a separate `git log`; this is the SAME subject worker_merge previewed at step 1. QUEUED PATH ALSO CARRIES IT (card 7a1a76e9 DoD-1/DoD-2): `commitSubject` on this SYNC return is the rare fast case — the common case under gate-queue load is the `pending` return below, which never carried it. The async `[loom:merge-done]` nudge now echoes it too (` subject=\"...\"`, right after \"merged.\"), and the settled `gate_status(opId)` read also carries it (see that tool's own description) — so the landed subject is reachable on every surface a queued merge is guaranteed to reach, not just the sync return this paragraph describes. GATE STEP DURATIONS (card a2873f7e): a merge that actually ran the gate (not reused, not gateless) ALSO carries `gateSteps: [{step, durationMs, status}, ...]` — one entry per `&&`-separated gate step — echoed into the `[loom:merge-done]` notification as a line prefixed `steps (diagnostic only — not a pass/fail signal): ...`. `steps` is the SAME shape on a rejection's `gateDetail` too, so a step's duration is comparable across a green run and a red one. This is DIAGNOSTIC ONLY — a prompt to look, never evidence to assert or threshold against; a duration swings with host load and a suite that silently runs less finishes EARLY, which reads as good news, not a red flag. GATE CONCURRENCY ON A PASS TOO (card e2b6f900): the same two dominant outcomes (a plain gate-fail rejection, or a plain successful merge) ALSO carry `gateCap`/`concurrentGates`/`concurrentGatesMax` — the concurrency this merge's gate ran under, persisted durably and readable via `gate_status(opId)` just like `outputTail`/`gateSteps` above; before this card only the rejection baked this as unstructured text into its own notification. `gate_status(opId)`'s own description carries the full span caveat (`concurrentGates` understates, `concurrentGatesMax` overstates) — read it there before comparing runs. NESTED-REPO SAFETY: after a successful merge, the worktree is force-removed (it always carries expected ephemeral untracked content — node_modules, dist, build caches). If the worktree ALSO contains a nested git repository (a subdirectory with its own `.git` — e.g. something cloned into it, which can hold real unpushed work), the removal is REFUSED and the worktree is RETAINED intact — the merge itself already landed, only the cleanup is deferred; the result carries {warning} naming the nested path(s). Move/push that content out yourself and re-confirm, or pass forceRemoveWorktree:true if you've confirmed the nested content is disposable (default false — the safe choice) — a forceRemoveWorktree:true call ALWAYS runs for real (see below), so this is also how you retry the nested-repo case even moments after the first confirm. WORKTREE-GC WARNING (task 035fb673): distinct from the nested-repo case above — the worktree removal itself came back wedged (a held OS handle survived a force-kill), left-on-disk (a transient handle-lag outlasted a few short retries), or needsHuman (wedged past a long give-up bound) — carries {warning} naming the path and what to do (this is non-blocking: the merge itself already landed either way, and this used to be console.warn-only, invisible here). CLIENT-TIMEOUT RESILIENT: a fast confirm returns {merged,...,opId} (stamped with a correlation `opId`); a slow one (the gate genuinely takes a while) instead returns {opId,status:\"pending\",workerSessionId} — rather than polling, wait for the async `[loom:merge-done]`/`[loom:merge-rejected]`/`[loom:merge-failed]` nudge, which carries this SAME opId (plus the worker/task) so you can match it to this call even with several merges pending at once. If you need the answer sooner, poll the READ-ONLY `gate_status(opId)` (never starts a new run) or worker_list (this worker's `pendingMerge` field). RE-CALLING worker_merge_confirm with the SAME workerSessionId is ALSO always safe, at ANY delay — idempotent-retryable like worker_spawn: a re-call while the gate/merge is STILL running attaches to that SAME in-flight op (never a second one), and a re-call landing AFTER it settles, at the SAME commit — seconds, minutes, or hours later, it makes no difference — gets the SAME cached verdict instead of starting a fresh gate run (worker_list's `pendingMerge.state` may have already reverted to nothing by then — that's a cosmetic display window only, unrelated to this dedupe) — never throws 'already in flight'. If the branch has picked up new commits since the cached verdict, a re-call gates them for real instead of serving the stale answer — that is answering a different question, not a silent re-run of the old one. NOTABLY, a branch that was behind main is itself advanced by THIS call's own pre-gate union-merge, so a later re-call gates the NEW base for real rather than returning a cached verdict — being behind main structurally means the cached-verdict guarantee above cannot apply to THAT re-call (card 615967c5). ANNOUNCING A FRESH GATE RUN (card 615967c5): whenever a call — pending OR settled-fast — actually ran a genuinely fresh gate instead of being served a cached verdict, the result ALSO carries `supersededBy: \"identity-mismatch\"|\"forced\"|\"genuinely-new\"` (`identity-mismatch` = a cached verdict existed but its identity didn't match, per the paragraph above — an OBSERVED field, never an assertion of cause: it names that the branch's identity changed, never WHY — main advancing under it, a sibling's squash landing, and the worker itself pushing a new commit all collapse into this same value, indistinguishably; `forced` = you passed forceRemoveWorktree:true; `genuinely-new` = no cached verdict existed for this worker yet in THIS daemon process — the cache is process-local, so a daemon restart also produces `genuinely-new` even for a branch checked before) plus, when known, `cachedVerdictIdentity` (the identity the superseded/bypassed cached verdict was validated against) and `currentIdentity` (this call's own freshly-resolved identity) — OBSERVED fields, not an assertion of cause. CACHE-HIT ANNOUNCEMENT (card 4aedde84, closing a real incident: a manager re-fired a merge after a gate failure and announced a re-run to a peer that had never happened — nothing distinguished the replayed verdict from a fresh one except the ABSENCE of `supersededBy`, and absence is not something a caller reliably notices): a settled result served from the cache instead ALWAYS carries a POSITIVE `servedFromCache: true` marker, never left to be inferred from `supersededBy` being missing — plus a `note` naming the opId being replayed, the identity that verdict was validated against when known, and that `forceRemoveWorktree:true` is the only path that re-runs an unchanged commit. `servedFromCache` and `supersededBy` can never both appear on the same result (they are decided by the SAME cache-hit-vs-fresh-mint fork inside the registry, never two independent checks) — check for `servedFromCache` directly rather than for the absence of anything. To force a genuine RE-RUN of the SAME commit — retrying a flake, or completing a nested-repo worktree cleanup — pass forceRemoveWorktree:true: a deliberate, named, one-shot escalation that ALWAYS runs for real, bypassing any cached verdict from an earlier call (so re-confirming WITH it set is the correct way to retry the identical commit — whether moments after a plain confirm, e.g. the nested-repo case above, or long after).",
      ...
      async ({ workerSessionId, forceRemoveWorktree }) => {
        try {
          ...
          // from a genuinely fresh gate run (never a cache hit — see PendingOpRegistry's own doc), on BOTH
          // branches below (pending AND settled) — a caller must never read either shape as a replayed
          // cached verdict when this is present. Absent entirely on a cache hit (settled branch below stays
          // byte-identical to before this card in that case).
          const freshMintFields = freshMintResponseFields(r.freshMint);
          if (!r.settled) {
            const note = r.freshMint
              ? freshMintNote(r.freshMint)
              : "gate/merge still running — poll gate_status(opId) (read-only, never starts a new run) or worker_list (this worker's pendingMerge field). Re-calling worker_merge_confirm with the SAME workerSessionId is also safe at any delay: a re-call at the SAME commit returns the cached verdict; a re-call after new commits on the branch gates them for real — that is answering a different question, not a silent re-run. To force a re-run at the SAME commit (e.g. retrying a flake), pass forceRemoveWorktree:true.";
            return ok({ opId: r.op.opId, status: "pending", workerSessionId, ...freshMintFields, note });
          }
          // CACHE-HIT ANNOUNCEMENT (card 4aedde84): `r.cacheHit` is the mirror of `r.freshMint` above,
          // present ONLY on a settled result served from the registry's cache (never alongside freshMint —
          // see CacheHitInfo's own doc) — turns it into a POSITIVE `servedFromCache` field plus a note, so a
          // caller never has to notice the absence of `supersededBy` to learn that no gate ran.
          const cacheHitFields = cacheHitResponseFields(r.cacheHit);
          if (!r.ok) {
            const errNote = r.cacheHit ? cacheHitNote(r.cacheHit, undefined) : undefined;
            return ok({ error: r.error instanceof Error ? r.error.message : String(r.error), ...cacheHitFields, ...(errNote ? { note: errNote } : {}) });
          }
          const note = r.cacheHit ? cacheHitNote(r.cacheHit, r.value.opId) : undefined;
          return ok({ ...r.value, ...freshMintFields, ...cacheHitFields, ...(note ? { note } : {}) });
        } catch (e) {
          return ok({ error: (e as Error).message });
        }
      },
    ...
    server.registerTool(
      "merge_batch",
      {
        description: "PRE-MERGE TITLE CHECK (card a32533a1): this call NEVER rewrites or coerces a candidate's commit subject — each branch's own commit(s) land verbatim (see below). Before calling this, check `worker_merge`'s `ownTipSubject`/`ownTipSubjectConventional` fields for each candidate — that's the exact, uncoerced subject for the branch's TIP commit only, byte-for-byte what this call will commit for it. For a branch with a SINGLE commit (the common case) the tip IS the branch's whole contribution, so that field alone is a complete check. For a branch with MORE THAN ONE commit it is NOT — every commit on that branch lands with its own original subject. `worker_merge`'s result also carries `ownNonTipCommitSubjects` (card 591906ae) — every one of THOSE other commits' own subjects, bounded — plus `ownNonTipCommitSubjectsConventional`; check both before batching a multi-commit candidate (no need to hand-run `git log` yourself). A branch that fails this check merges solo instead — the solo squash path rewrites the subject to the card title, so a bad worker-authored subject never reaches mainline that way. Card dbc6f660 — batch the merge gate: gate up to the project's configured `orchestration.maxConcurrentWorkers` (Settings UI: \"Max workers / manager\", resolved fresh via resolveConfig, platform default 3) READY worker branches on ONE repo in a SINGLE run, instead of one gate per branch. This cap is FIXED, not a parameter of this tool — batch size is deliberately not agent-settable. Cuts a dedicated batch worktree off canonical main's current tip and lands each candidate branch into it IN ORDER, EACH BRANCH'S OWN COMMITS INDIVIDUALLY (card 6801c0a1 — a rebase/cherry-pick, NOT a squash: a branch contributing 3 commits lands 3 commits on main, not 1; this is DIFFERENT from a solo worker_merge_confirm, which still squashes a branch to one commit). No merge commits are ever created. Only the branch's LAST (tip) commit carries the `Loom-Worker-Branch` trailer — its earlier commits land with their own original messages and no trailer at all. Runs the gate ONCE on the assembled tree, and on green fast-forwards canonical main to it (main is mutated exactly once, at that fast-forward). Card d422e279: that ONE gate run reduces to build + static guards + any changed test file(s) — the SAME smaller command a solo worker_merge_confirm already substitutes — when the ASSEMBLED tree's own union of changes proves eligible (e.g. every candidate is test-only); `chosen` batch MEMBERSHIP is unaffected either way (dbc6f660 already ruled out excluding an individually-reduced-eligible branch from a batch, and that stands — this is a DIFFERENT decision, about what the one resulting gate run itself executes). ⚠️ CARD fd0d34da — THE K-DEGRADATION THIS REDUCTION HAS, BY CONSTRUCTION, AS BATCH SIZE GROWS: the union's own reduce-eligibility check is a SINGLE `git diff --name-status` classification loop over the WHOLE assembled tree that RETURNS on the FIRST changed path outside its four scopes (`packages/daemon/{src,test,assets,scripts}/`) — a `return`, never a `continue` — so ONE stray path outside its four scopes AND NOT ALREADY CERTIFIED INERT (`docs/**`, the root `README.md`/`CHANGELOG.md`/`CODE_OF_CONDUCT.md`/`CONTRIBUTING.md`/`SECURITY.md` set) — e.g. `packages/shared/**`, `packages/web/**`, or any OTHER root-level file such as `CLAUDE.md` — IN ANY ONE of the batched branches voids reduction eligibility for the ENTIRE union, even when every other branch in the batch is itself individually reducible. Because the union's changed-path set only ever grows with the number of branches batched together, P(this reduction is inapplicable) rises with batch size for a reason that has nothing to do with whether any individual branch's own change was reducible. ⛔ THIS IS NOT A DEFECT TO WORK AROUND: the `return` is correctly fail-closed — an unclassifiable path means the assembled diff cannot be proven emit-identical to main, and weakening it to a `continue` (skip the unclassifiable path and keep checking the rest) would let an unverified change through a reduced gate. When this reduction fails to apply, `gate_history`'s row for this batch reads `emitCompareReduced:null` with `emitCompareNotApplicableKind` set to `\"repo-out-of-domain\"` or `\"path-out-of-scope\"` (see that tool's own description for the distinction) — a `null` here is routine at larger batch sizes, not a signal something is wrong with the batch mechanism itself. A landed result carries `reducedGateWarning` (sync return or the async settle nudge) ONLY when this happened — it names any NOT_HERMETIC/inert-path exclusions and warns that the green covers every landed branch via test files run in ISOLATION (`--only=`), same isolation caveat a solo reduced merge already carries. Every workerSessionId must be YOUR OWN worker, on the SAME repo — a mismatch refuses the whole call before touching anything. Fewer than 2 eligible candidates (after stranded-work/cap filtering) is not worth batching and falls straight to the ordinary per-branch path. A branch that won't land cleanly into the batch (a real conflict on any of its own commits against an earlier candidate in this SAME batch, or a branch carrying a merge commit that is NOT a pure main-forward — one whose non-first parent is not already on the batch HEAD, or that carries conflict-resolution content of its own; a clean automatic main-forward merge, e.g. the union-forward a solo worker_merge_confirm commits onto the branch even if you later cancel it, is skipped and the branch's own commits land normally) is DROPPED WHOLESALE, never partially landed — recorded in `fallback` with ITS OWN reason (each `fallback[]` entry also carries `started` — card 2c16447b: `true` means its per-branch worker_merge_confirm was actually kicked off, `false` means it was NOT started and `reason` says why, e.g. the worker is no longer in this manager's recycle lineage; a `started:false` candidate needs a manual worker_merge_confirm — the nudge counts only started ones as \"routed\") (naming the merge sha and which condition failed, or the conflicting commit) — also carried, bounded, in the [loom:merge-batch-*] nudge and filed as a durable `batch_merge_dropped` event per drop — and the batch proceeds with the rest; the dropped branch then gets its own ordinary worker_merge_confirm (which still squashes, unaffected). A RED gate on the assembled batch first tries card 67030bb9's bounded multi-file retry (mirrors worker_merge_confirm's own single/multi-file retry exactly — see gate_status's `retriedFile`/`retryPassed` doc for the mechanism, and gate_history's `retryDeclineReason` doc for why a failure sometimes declines it): if the failure names a small, identifiable set of re-runnable test files, THOSE are re-run together in isolation once before the batch is declared red — a resulting pass lands the WHOLE batch as a retry-assisted pass, WEAKER evidence than a clean pass (never bisected file-by-file; every landed branch shares the same one retry-assisted verdict). Only a gate that stays red (including one this retry declined or couldn't rescue) falls back to every original candidate being gated individually, as before this card (this remains measured cheaper than bisection at this batch-size range). A FORFEIT (canonical main advanced between the batch being cut and the fast-forward — a human or another writer landed something mid-gate) is also a full fallback to individual gating, and is recorded as a distinct `batch_merge_forfeited` event — this is the one failure mode batching makes strictly worse than today (one branch's gate wasted becomes up to the cap), so watch for it if you batch large groups often. CANDIDATE ORDER THROUGH THE FALLBACK (card 99a1cf6f) is PATH-DEPENDENT, not universal: on a RED gate or a FORFEIT, `fallback` routes candidates through in your ORIGINAL resolved batch order (the order you passed them in, after ownership/repo/stranded-work filtering) — a deliberate landing order (e.g. sequencing a lock-fix branch first) IS preserved there. On a GREEN batch's own dropped/conflicted candidates, it is NOT: those route through in the order `assembleBatchBranches` actually attempted them, which is sorted OLDEST-AUTHOR-DATE-FIRST for the batch's own internal landing, not your original call order — if you need a specific fallback order for dropped candidates specifically, don't assume it matches how you passed `workerSessionIds`. FALLBACK CANDIDATES ON THE SAME REPO CAN OVERLAP (card 99a1cf6f) because kickoff order is not the same as execution order: `fallback`'s own per-candidate `worker_merge_confirm` calls are dispatched with only a short (~12s) stagger between them, well under a real gate's own multi-minute run, so more than one can be genuinely gate-running at once. This is FAIL-CLOSED, never a data-loss risk — a losing candidate self-aborts at squash time with a benign `gate_base_invalidated`/`\"stale-base\"` rejection (see `worker_merge_confirm`'s own description) rather than landing unverified content, and a plain re-confirm on it genuinely re-gates. The real cost is THROUGHPUT: a batch that falls back to K individual merges can burn MORE than K full gate runs when two or more overlap and one forfeits — factor that into how large a group you batch, especially when candidates are likely to conflict/drop together. `landed` names every branch that actually merged via the shared batch gate (workerSessionId, taskId, branch, sha, strippedTrailerCount — `sha` is the branch's TIP commit, the one carrying its trailer; `strippedTrailerCount` (card b7f965d2) is how many of THAT branch's own commits carried a `Claude-Session:` harness attribution trailer that got stripped before landing — 0 in the ordinary case, non-zero means a worker put that trailer on its own commit against this project's CLAUDE.md, worth a look) — each one already went through the SAME finalize path (worktree cleanup, task move, notification) a solo ALREADY_MERGED confirm uses, no separate action needed. `fallback` names every OTHER candidate (dropped, over the cap, stranded, or — on a red gate/forfeit — the whole batch) that this call already routed through an ordinary worker_merge_confirm on your behalf — you do not need to re-call it yourself, just read `fallback` to see what happened to each. `reason` is set on a top-level refusal (validation failure, nothing eligible, no gateCommand configured) or to explain why the whole batch went to fallback. " +
          "CLIENT TIMEOUT ≠ FAILURE, AND A RE-FIRE IS NOW CHEAP (card f944d4e4 — supersedes 46ebdf20's now-stale advice below, which used to say the opposite): like `worker_merge_confirm`, this call is now BOUNDED — a slow batch (worktree cut, assembly, gate, fast-forward) returns `{opId, status:\"pending\"}` well before the batch itself settles, instead of awaiting it synchronously, so a client-side timeout does NOT mean the batch failed. To check on a pending op, read `gate_status(opId)` (read-only, never starts a new run) or `gate_queue` for a `merge` row with `taskId:null`/`branch:null`/`workerLabel:\"Orchestrator\"` — a solo merge always carries a real `taskId`/`branch`, so those nulls (not their absence) are the discriminator for a live batch's OWN gate row specifically. ⚠️ CARD 19256231 — THAT PREDICATE FINDS ONLY THE BATCH'S OWN GATE, NEVER ITS FALLBACK: the automatic per-branch fallback this call already routes `fallback` candidates through (see `fallback`'s own doc above) — on a REJECTED batch (a red gate or a forfeit), EVERY chosen candidate; on a GREEN batch, its own dropped/over-cap/stranded candidates — spawns one ordinary `worker_merge_confirm`-shaped merge gate PER CANDIDATE — each with a REAL `taskId`, a REAL `branch`, and that worker's own `workerLabel`, i.e. the exact opposite shape the `taskId:null`/`branch:null`/`workerLabel:\"Orchestrator\"` predicate looks for. A caller who sees the batch row settle and finds no `Orchestrator` row correctly concludes the BATCH is done, while up to the batch's own candidate count worth of individually-gated merges it never separately authorized may still be live on the daemon-global shared cap. Each such fallback row DOES carry a distinguishing signal: `gate_queue`'s `fallbackOfBatchOpId` field, present (and equal to THIS op's own `opId`) on every merge row this batch's fallback spawned, `null`/absent on an ordinary, non-batch-spawned merge — read it instead of widening the `taskId:null`/`branch:null` predicate, which would only ever match the batch's own row regardless. `gate_status(opId).state===\"settled\"` for a batch op is now SAFE TO POLL FOR AND ACT ON (card 81d795de): it means the batch operation as a whole has finished, WHATEVER its outcome — nothing (the shared gate run, the fast-forward onto canonical main, or any per-branch finalize) is still running — never merely that the gate command itself finished. On a RED gate or a FORFEIT, no fast-forward and no finalize ever run at all, and the row still settles; do not read \"settled\" as \"main advanced\", only as \"nothing about THIS op's own work — worktree cut, shared gate run, fast-forward, already-landed-branch finalize — is still in flight\" (⚠️ CARD 19256231: NOT \"nothing is still in flight, period\" — see the correction immediately below for the one thing that claim leaves out) — read the actual result (sync return, async settle nudge, or `gate_status`'s own verdict fields) for the outcome. ⚠️ CARD 19256231 CORRECTION — `settled` DOES NOT wait for a fallback candidate's own merge to FINISH, only for this call to have SPAWNED it: `runFallback` routes every fallback candidate (dropped, over-cap, stranded, or a whole red-gated/forfeited batch) through an ordinary `worker_merge_confirm` on your behalf, SEQUENTIALLY — but each of those is itself a bounded, attach()-backed call that returns as soon as EITHER it settles inline OR its own short sync-wait budget elapses (whichever is first), so `runFallback`'s loop — and therefore this whole op's own settle — can complete well before a fallback candidate's underlying gate/squash has actually finished. A large fallback set can therefore report THIS op `settled` while its own individually-gated merges are still genuinely running — read `gate_queue`'s `fallbackOfBatchOpId` field (or poll each fallback candidate's OWN `worker_merge_confirm`/`gate_status`) for their real, independent completion; do not treat this op settling as proof they have. RE-FIRING `merge_batch` with the SAME workerSessionIds is SAFE AND CHEAP: it is keyed by the calling manager plus the RESOLVED candidate set (after ownership/repo/stranded-work filtering, i.e. the set that will actually be gated together) — a re-call whose resolved set is unchanged re-attaches to the already-running (or just-settled) op instead of cutting a second batch worktree, re-assembling candidates a second time, or running a second gate. One named residual: if the resolved set itself changes between calls (a candidate drops out via ownership/repo/stranded-work in between) the key differs and the retry runs fresh — believed rare for an ordinary client-timeout retry (a `done`+`awaitingReview` candidate's resolution doesn't usually change moment to moment), but real, not closed by more machinery. An async settle nudge (`[loom:merge-batch-done]`/`[loom:merge-batch-failed]`/`[loom:merge-batch-unknown]`) fires once, but ONLY for a caller that actually observed the pending shape — a caller whose batch settles inside the bounded wait gets the full result inline and needs no nudge.",
    ...
    server.registerTool(
      "gate_history",
      {
        description:
          ...
          "emitCompareReduced, emitCompareNotApplicableKind, emitCompareIdenticalCount, emitCompareTestFiles, fallbackOfBatchOpId}. " +
          ...
          "`emitCompareNotApplicableKind` (card fd0d34da) is the coarse, PATH-FREE reason `emitCompareReduced` " +
          "reads `null` FOR THIS ROW specifically because the predicate said `notApplicable:true` — " +
          "`null` for every OTHER cause of a `null` `emitCompareReduced` (no gate spawned, a pre-fd0d34da " +
          "row, a `\"worker\"`/`\"deploy\"` row, or a genuine `false`). One of `\"repo-out-of-domain\"` " +
          "(a REPO-LEVEL fact, checked via a dedicated `git ls-tree` against the repo's own tree rather than " +
          "inferred from this diff alone — NONE of the four scope directories exist anywhere in this repo, " +
          "so the predicate could never decide ANY diff on it) or `\"path-out-of-scope\"` (covers BOTH " +
          "remaining shapes, sharing the identical actionable fact that the predicate applies to this repo " +
          "fine, just not — fully, or at all — to THIS diff: either this diff touched an out-of-scope path " +
          "while the SAME diff also touches an in-scope one elsewhere, or the repo's own tree DOES have one " +
          "of the four scope directories even though this particular diff doesn't touch it; see " +
          "`merge_batch`'s own description for the batch-size degradation this specific shape drives), plus the rarer " +
          "`\"harness-config-unavailable\"`/`\"typescript-unresolvable\"`/`\"git-operation-failed\"`/" +
          "`\"empty-diff\"`/`\"unparseable-diff\"` mechanism-failure reasons. Before this card every one of " +
          "these collapsed onto the same bare `null`, indistinguishable without re-running the predicate " +
          "against the diff by hand. " +
          ...
          "Card 344ce950 (single-file) / 67030bb9 (bounded multi-file, AND — since 67030bb9 — a batch row, " +
          "`detail.batched === true`, not just a solo one) — `retriedFile`/`retryPassed`: a " +
          "`gateType:\"merge\"` row whose test step named UP TO `MULTI_FILE_RETRY_MAX` identifiable, " +
          "re-runnable files retried together, once, before this row's verdict — `retriedFile` is a " +
          "comma-joined list of the retried name(s) (a bare single name for the N=1 case, byte-identical " +
          "to before card 67030bb9), `null` on the overwhelming majority of rows (nothing retried). A " +
          "`\"pass\"` row " +
          ...
          "`retryDeclineReason` (card 67030bb9) is present ONLY when a genuine gate failure was eligible " +
          "for this retry mechanism to even CONSIDER (never on a `\"kill\"`/`\"timeout\"` attempt 1, which " +
          "the sibling TRANSIENT-KILL retry owns instead) but `identifyRetriableTestFiles` declined it — " +
          "one of its own decline-reason strings (`\"no-fail-tier-match\"`, `\"count-mismatch\"`, " +
          "`\"over-cap\"`, `\"harness-not-executed\"`, `\"unparseable-name\"`, `\"file-not-found\"`, " +
          "`\"duplicate-name\"`) explaining WHY, so a rejection like this is diagnosable after the fact " +
          "from THIS tool alone, instead of needing a fresh live repro. Mutually exclusive with a non-null " +
          "`retriedFile` on the same row — a retry is either identified (eligible for `retriedFile`) or " +
          "declined (eligible for THIS field), never both. `undefined` on the overwhelming majority of " +
          "rows — either `identifyRetriableTestFiles` was never even called (the outer genuine-failure " +
          "guard was already false) or this row predates the field; never read `undefined` as \"eligible\". " +
    ...
    server.registerTool(
      "daemon_restart",
      {
        description:
          ...
          "{restarting:true} on success, or {restarting:false, error} if unsupervised, a live restart " +
          "supervisor process can no longer be confirmed (card 83718377 — it re-checks the live OS process " +
          "table on every call, not just whether this process was ONCE spawned under one; refuses rather " +
          "than exit into an orphan nothing would relaunch — retry if the error says the CHECK itself " +
          "failed, ask a human to restart the supervisor if it says the supervisor is actually gone), or " +
          "build failed. If the " +
