    ...
    server.registerTool(
      ...
      async ({ taskId, agentId, kickoffPrompt, kickoff, reviewOfWorkerSessionId, reviewOfTaskId }) => {
        ...
        try {
          ...
          return ok({ workerSessionId: worker.id, branch: worker.branch, worktreePath: worker.worktreePath, ...shipped, ...dirty, ...discarded, ...stale, ...review, ...reused, capacity: worker.capacity });
        } catch (e) {
