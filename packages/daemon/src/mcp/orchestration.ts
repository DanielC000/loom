        ...
        try {
          ...
          if (!r.settled) {
            return ok({
              ...
              note: "batch still running — poll gate_status(opId) (read-only, never starts a new run) or gate_queue for a merge row with taskId:null/branch:null. state:\"settled\" now genuinely means nothing is still running for this op — the gate run, the fast-forward onto canonical main, per-branch finalize, AND every fallback candidate's own sequential worker_merge_confirm — not just that the shared gate command finished (card 81d795de); this can stay pending through multiple solo fallback gates' worth of wall time on a batch with a large fallback set, which is expected, not a stall. Re-calling merge_batch with the SAME workerSessionIds is also safe at any delay: as long as the resolved candidate set is unchanged, it re-attaches to this SAME in-flight (or just-settled) op instead of starting a second one.",
            });
          }
          if (!r.ok) return ok({ error: r.error instanceof Error ? r.error.message : String(r.error) });
          return ok(r.value);
