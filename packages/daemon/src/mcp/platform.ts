...
const orchestrationOverride = z.object({
  ...
  recycleAtTurnsNoTelemetry: z.number().int().min(0).optional(),
  ...
  private buildServer(callerSessionId?: string): McpServer {
    ...
    server.registerTool(
      ...
      async ({ projectId, scope, state, full, limit, offset }) => {
        ...
        const off = offset ?? 0;
        const projected = projectSessionList(filtered, { full, limit: effLimit, offset });
        // `lastEngineOutputAt` isn't a `SessionListItem` field (a live, in-memory-only signal — see
        // PtyHost.getLastOutputAt's own doc) so it can't ride the shared, pure sessionView.ts projection;
        // enrich it here, the one call site with `pty` access. CLAUDE-ONLY (card a1916267): a codex row
        // always reads `null` here, an honest "not applicable to this harness" absence — never read a
        // codex row's null as "dead"/"wedged", and never as evidence the process isn't live at all.
        const page = projected.map((row) => ({ ...row, lastEngineOutputAt: pty?.getLastOutputAt(row.id) ?? null }));
        const returned = page.length;
        // nextOffset mirrors session_transcript's pageTranscript convention exactly: offset+returned while
        // more remains under the SAME effective limit, else null — never set when effLimit is unbounded
        // (full:true with no explicit limit already read everything there is).
    ...
    server.registerTool(
      ...
      async ({ kind, projectId, sessionId, taskId, limit, offset, fields, countsOnly }) => {
        ...
        if (countsOnly) return ok(eventsCountQuery(db, { kind, projectId: resolvedProjectId, sessionId, taskId }));
    ...
    server.registerTool(
      ...
      async ({ projectId, name, vaultPath, repoPath }) => {
        ...
        return ok({ ...projectFields(db.getProject(projectId)), staleStartupPrompts });
      },
    );
