  ...
  private buildServer(projectId: string, sessionId: string): McpServer {
    ...
    if (session?.role !== "assistant") {
      ...
      server.registerTool(
        ...
        async ({ id, taskId, baseVersion, allowTruncate, ...patch }) => {
          ...
          return ok(await updateProjectTask(db, projectId, resolvedId, patch, { sessionId, role: session?.role }, baseVersion, allowTruncate));
        },
      );
      server.registerTool(
        "tasks_defer_item",
