  ...
  private buildServer(projectId: string, sessionId: string, attributions?: Map<string, ToolAttributionResult>): McpServer {
    ...
    if (session?.role !== "assistant") {
      server.registerTool(
        ...
        async ({ allowDuplicate, supersedes, relatedTo, allowHtmlEntities, ...args }) => {
          const created = createProjectTaskChecked(db, projectId, args, { allowDuplicate, supersedes, relatedTo }, allowHtmlEntities);
      ...
      server.registerTool(
        ...
        async ({ id, taskId, baseVersion, allowTruncate, appendBody, allowHtmlEntities, ...patch }) => {
          ...
          const result = await updateProjectTask(db, projectId, resolvedId, patch, { sessionId, role: session?.role }, baseVersion, allowTruncate, appendBody, allowHtmlEntities);
