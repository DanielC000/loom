...
export class TaskMcpRouter {
  ...
  private buildServer(projectId: string, sessionId: string, attributions?: Map<string, ToolAttributionResult>): McpServer {
    ...
    server.registerTool(
      ...
      {
        description:
          "List this project's board tasks. Returns NEWLINE-DELIMITED JSON — one task object per line, NOT a JSON array — so a wide read stays Read/grep-pageable even if it spills to a file. Above ~" + SPILL_INLINE_BUDGET_CHARS + " chars the rows are written to a scratch file instead of inlining, and the response becomes a `{rowsFile,rowsChars,rowCount,note}` pointer at that same NDJSON text (one task per line) — page it with Read or grep it; re-call with a narrower filter/limit to inline instead. DEFAULT: a lightweight SUMMARY ({id,title,columnKey,position,priority,updatedAt,merged,repoKey,deferred,deferredUntilTaskId,deferredStuck,deferredAt,deferredReason,held,heldBy}) — bodies OMITTED, terminal/done cards EXCLUDED. Pass includeBody:true for full bodies, or tasks_get(id) for one card. `held` is the owner's SOLE brake — `worker_spawn` refuses to dispatch onto a held:true card — and `heldBy` (`\"human\"|\"agent\"|null`) is server-stamped provenance of who set it; both are now projected here (card 245c0837), so triaging the board no longer requires a per-card tasks_get to see a hold. deferredUntilTaskId (when set) names the task(s) this card is auto-cleared behind — a bare id for one blocker, or an array for several (card 022659ac); deferred flips to false here once EVERY named blocker's merged state resolves non-null, no separate check needed (one of several landing does not by itself clear a card blocked on more than one). `deferredReason` is the reason required on a manual deferral (deferred:true with no deferredUntilTaskId) — null on an auto-deferral or when not deferred. `deferredAt` is the server-stamped instant the current deferral started — null when not deferred. `deferredStuck` (only meaningful while deferred:true) is `true` when that release condition can no longer be shown to resolve — the named blocker is deleted, or it's already sitting in the terminal (done) column with no proven merge (e.g. it closed with zero commits, a legitimate outcome that never produces a squash commit); with multiple blockers this is the OR across all of them — ANY one going dangling/closed-with-no-merge sets it, even while the others are still cleanly pending — a fail-toward-VISIBLE signal, not proof the blocker will never ship (a real merge outside the scan window reads the same way); a stuck card is worth a manual look rather than trusting it'll resolve on its own. `merged` is this card's git-derived ship state — {sha,date,verification?} of its squash-merge commit on this project's repo if one is found, else null; null means NOT PROVEN merged (never merged, landed outside the scan window, or a git read failure), not an authoritative 'never merged' — treat a predecessor's 'unbuilt'/'won't-do' claim as suspect if merged is non-null. `verification` names WHICH check answered it: \"content\" (byte-verified against a still-live branch tip — the strongest), \"pathset\" (verified from the landed commit's own ancestry against a persisted path-set trailer — proves the same FILES landed, not the same content), or \"trailer-only\" (pre-fix history — trailer PRESENCE alone, the weakest). Absent means unknown, not unverified. `merged.verification` is recomputed FRESH on every read — a full row (includeBody:true) ALSO carries `mergedVerificationAtMerge` (card 634edd2b), the verification mode recorded ONCE when this card's merge landed and never re-derived since; the two can legitimately disagree once the branch is later deleted (live content-verification then degrades to pathset/trailer-only, while the frozen at-merge value doesn't move) — never treat a disagreement between them as a bug, and never read either as more current than the other without checking which one you actually need. `mergedVerificationAtMerge:null` on a MERGED card (card d6d40edd) means one of: a solo ALREADY_MERGED/boot-reconcile landing, a batch landing whose content was already present elsewhere in the batch (`noop`), or pre-fix history — NEVER an anomaly to chase; it backfills lazily only via the web board drawer (human-triggered, not agent-facing). A freshly-landed batch branch DOES stamp `\"pathset\"`/`\"trailer-only\"` eagerly, same as a solo squash-confirm stamps `\"content\"`. Filters: columns:[...] (only those column keys), excludeDone:false (include done), minPriority:p0|p1|p2|p3 (only tasks at or above it; lower number = higher priority), idPrefix (only ids starting with this), titleContains (case-insensitive title substring) — prefer a scoped filter over paging a huge window. fields:[...] (card 23fde5f8) projects each returned row down to ONLY the given top-level key names, e.g. fields:[\"id\",\"title\",\"columnKey\",\"priority\",\"held\",\"deferred\",\"merged\"] — the shape a board triage read actually needs, dropping the rest (including deferredReason, the field most likely to blow the inline budget on a wide read). An un-asked-for field is genuinely ABSENT from every row of the response, not merely dropped from a spill file preview — and an unmatched/unknown field name is silently ignored rather than erroring, so a typo just yields a smaller row. `id` is NOT auto-added — include it yourself if you need to correlate rows back to cards. Ignored (has no effect) when countsOnly:true is also passed, since that mode never materializes row bodies to project in the first place. Capped at " + DEFAULT_TASK_SUMMARY_CAP + " rows by default (the DEFAULT limit when you pass none) — page with limit/offset. COMPLETENESS SIGNAL (card 84f6ac42): with NO offset/limit passed and the whole matching set fits in one page, returns the bare NDJSON rows exactly as before (today's shape, unchanged) — otherwise, or whenever you pass offset/limit explicitly, it returns a page envelope alongside the rows instead: {rows,total,returned,offset,nextOffset} inline, or {rowsFile,rowsChars,rowCount,note,total,returned,offset,nextOffset} when the rows themselves spilled to a file. `total` is the TRUE matching-row count before this call's offset/limit slice; `nextOffset` is offset+returned while more remains, else null. Page deterministically by calling again with offset:nextOffset until it is null (mirrors list_all_tasks' own page envelope) — a capped read is thus self-evidently partial; a `rowCount`/`returned` equal to your `limit` never by itself means \"that's everything\" unless `nextOffset` is null. Pass countsOnly:true to answer \"how many cards, by column/priority\" WITHOUT fetching any row bodies — returns {total, byColumn, byPriority} (a few hundred bytes) instead of the filtered row set; the same column/priority/id/title filters above still apply, limit/offset/includeBody are ignored in this mode.",
      ...
      async (args) => {
        ...
        // Card 23fde5f8: project down to the requested fields AFTER the completeness signal (total/
        // nextOffset) is computed against the full row set — `fields` narrows what's IN each row, never
        // how many rows there are. `effective` (which the spill key hashes) already carries `fields`, so
        // two different projections of the same query never collide on one spill file.
        const projected = pickFields(rows, effective.fields);
        const result = okLinesSpillable(sessionId, "tasks-list-spills", taskListSpillKey(effective), projected, { total, offset: off, nextOffset, explicit });
    ...
    server.registerTool(
      "tasks_get",
      {
        description: "Read ONE full task (title + body) by id; project-scoped. id accepts the full id OR an unambiguous 8-char id-prefix (mirrors project_get). `taskId` is accepted as an ALIAS for `id` (matches the taskId param name every sibling task tool uses) — pass either one (if both, id wins). An optional `projectId` is tolerated but ignored — this tool is already scoped to the caller's own project. Also returns `deferredStuck` (only meaningful while deferred:true) — see tasks_list's own description for the full contract; it's the same field. Also returns a `requests` summary ({total, answered, pending, cancelled, items:[{id,type,title,state}]}) of any Requests connected to this task (soft-linked via taskId at question_ask time) — a task you're working may already carry a prior owner decision you'd otherwise miss; read one in full via task_request_get, or list them all via task_requests_list. Also returns this card's OWN outbound `deferredItems` ([{id,text,toTaskId,status,createdAt,updatedAt}], card 0d4bc3f0) — sub-items THIS card's DoD deferred onto other cards via `tasks_defer_item` — AND `incomingDeferredItems` ({total,open,acknowledged,declined,items:[{itemId,text,status,fromTaskId,fromTaskTitle,createdAt,updatedAt}]}), the INBOUND view: every item ANY other card on this board has deferred onto THIS one, regardless of that donor card's own state. Read `incomingDeferredItems.open` before assuming a card with no obvious open work is actually idle — a hand-off nobody has acknowledged shows up here even if the donor card has already closed. Flip an item's status with `tasks_defer_item_ack`. Also returns `merged` — this card's git-derived ship state ({sha,date,verification?} of its squash-merge commit on this project's repo, else null). null means NOT PROVEN merged, never an authoritative 'never merged' — don't trust a stale handoff claiming this card is unbuilt without checking this first. `verification` names WHICH check answered it: \"content\" (byte-verified against a still-live branch tip — the strongest), \"pathset\" (verified from the landed commit's own ancestry against a persisted path-set trailer — proves the same FILES landed, not the same content), or \"trailer-only\" (pre-fix history — trailer PRESENCE alone, the weakest). Absent means unknown, not unverified. `merged.verification` is recomputed FRESH on every read — this response ALSO carries `mergedVerificationAtMerge` (card 634edd2b), the verification mode recorded ONCE when this card's merge landed and never re-derived since; the two can legitimately disagree once the branch is later deleted (live content-verification then degrades to pathset/trailer-only, while the frozen at-merge value doesn't move) — never treat a disagreement between them as a bug. `mergedVerificationAtMerge:null` on a MERGED card (card d6d40edd) means one of: a solo ALREADY_MERGED/boot-reconcile landing, a `noop` batch landing (content already present elsewhere in the batch), or pre-fix history — NEVER an anomaly to chase; it backfills lazily only via the web board drawer (human-triggered, not agent-facing). A freshly-landed batch branch DOES stamp `\"pathset\"`/`\"trailer-only\"` eagerly, same as a solo squash-confirm stamps `\"content\"`. `fields:[...]` (card 40f4cae9, same contract as `tasks_list`'s own `fields`) projects the returned task down to ONLY the given top-level key names, e.g. fields:[\"id\",\"title\",\"columnKey\"] — an un-asked-for field is genuinely ABSENT from the response, not merely dropped from a spill preview, and an unmatched/unknown field name is silently ignored rather than erroring. `id` is NOT auto-added — include it yourself if you need it. Applied AFTER the spill decision (a `body`/`bodyFile` split still happens on the full, unprojected content) so an oversized body is still safely spilled even when `fields` doesn't ask for it, then the (possibly spill-pointer-carrying) result is projected down to the requested keys. `bodyGrep`/`bodySlice` (card 67c54f48, item 3 — replaces the \"grep it... slice it via Bash\" workaround the spill `note` below used to be the only answer to) excerpt `body` SERVER-SIDE, applied BEFORE the spill decision above, so a small excerpt request usually never even reaches it. Pass at most one: `bodyGrep` (a case-insensitive plain-substring match, never a regex) returns only the matching lines, newline-joined, plus `bodyGrepMatches` (the match count — 0 means \"no match\", not \"empty body\"); `bodySlice` ([start,end), `String.slice` semantics — an out-of-range end is silently clamped, never an error) returns that character range of `body`. Either way the response carries `bodyExcerpt:true` and `bodyCharsFull` (the ORIGINAL body's length before excerpting), and `body` (or `bodyFile`, if the excerpt itself still spills) holds only the excerpt — passing both is an error.",
        inputSchema: strictShape({
          id: z.string().optional(), taskId: z.string().optional(), projectId: z.string().optional(), fields: z.array(z.string()).optional(),
          bodyGrep: z.string().optional(),
          bodySlice: z.tuple([z.number().int().nonnegative(), z.number().int().nonnegative()]).optional(),
        }),
      },
      async ({ id, taskId, fields, bodyGrep, bodySlice }) => {
        const resolvedId = id ?? taskId;
        if (!resolvedId) return ok({ error: "id (or taskId) is required" });
        if (bodyGrep !== undefined && bodySlice !== undefined) return ok({ error: "pass either bodyGrep or bodySlice, not both" });
        const result = await getProjectTask(db, projectId, resolvedId);
        if ("error" in result) return ok(result);
        // Card 67c54f48: excerpt BEFORE spilling — spillableTaskGet then sees the (usually much smaller)
        // excerpted body, same as it always sees whatever `body` currently holds.
    ...
    if (session?.role !== "assistant") {
      server.registerTool(
        "tasks_create",
        {
          description: "Create a task on this project's board. priority p0|p1|p2|p3 (low number = higher priority), default p2. Optional repoKey (multi-repo epic) targets one of this project's registered `repos` — omit (or pass \"primary\") for the project's primary repo; an unknown key is rejected with {error}. CROSS-CHANNEL DUPLICATE ADVISORY (card 5b221bf2, demoted from a block to advisory-only by card d6890435): if this card's title+body shares a RARE STRONG identifier (a session id or a Loom branch name — full UUIDs/`loom/<hex>` only, NOT prose/title similarity, and NOT a bare code symbol/file:line/error constant on its own) with an existing card on this board, the create still SUCCEEDS and the result carries a `related: {taskId, title, sharedIdentifiers}` field naming the suspected counterpart — never a block, never a silent drop or auto-merge. Pass allowDuplicate:true to skip computing the advisory (\"I know, don't tell me\"), or supersedes/relatedTo:\"<taskId>\" (full id or unambiguous prefix) to instead declare an explicit relationship, noted on the new card's body (and back-noted on the target's). HTML-ENTITY GUARD (card 267fd215): a title containing &lt;/&gt;/&amp;/&quot;/&#NN; is REFUSED with {error} naming the decoded form you likely meant — a SOLO merge uses the title VERBATIM as the squash commit subject, so an accidentally-escaped title becomes a PERMANENT mainline artifact. If the title is genuinely ABOUT escaped HTML (not an accidental artifact), pass allowHtmlEntities:true. CONVENTIONAL-TYPE GUARD (card 3a833d94): a title carrying a `type(scope):`-shaped prefix whose type is NOT one of feat/fix/docs/style/refactor/perf/test/build/ci/chore/revert is REFUSED with {error} naming the bad type and the allowed set — the merge-time coercion net only fixes BARE prose (no such prefix at all), so an already type-shaped-but-invalid prefix would otherwise ship verbatim as a permanent mainline commit subject. A title with NO type-shaped prefix at all is unaffected (that's the bare-prose case the coercion net already handles correctly). Pass allowNonConventionalType:true if this is deliberate.",
          inputSchema: strictShape({
            ...
            allowHtmlEntities: z.boolean().optional(),
            allowNonConventionalType: z.boolean().optional(),
          }),
        },
      ...
      server.registerTool(
        "tasks_update",
        {
          description: "Update a task by id; project-scoped. PATCH-style: pass only the field(s) you're changing. id accepts the full id OR an unambiguous 8-char id-prefix (mirrors project_get); `taskId` is accepted as an ALIAS for `id` (matches the taskId param name every sibling task tool — tasks_get/task_requests_list/task_request_get — uses) — pass either one (if both, id wins). priority p0|p1|p2|p3 (low number = higher priority). held=true marks an owner-gated card the idle watchdog won't nag about — you MAY set this yourself. held=false CLEARS it, but only if held wasn't set by the owner: clearing an owner-set hold is REFUSED here (returns {error}, nothing written) — only the owner can release their own hold, via the board UI. deferred=true is YOUR OWN sequencing/dependency-gating marker — also discounted from the idle watchdog's actionable count, but (unlike held) never blocks worker_spawn. Optional deferredUntilTaskId (\"deferred until THIS task merges\") pairs with deferred:true for a blocker that's a queryable card ON THIS BOARD (full id or unambiguous prefix, normalized to the full id) — the daemon then auto-clears deferred the next time this card is read, once that blocker's `merged` state resolves non-null; pass null to clear the pairing. Card 022659ac: pass an ARRAY of ids instead of one when the card is genuinely blocked on MORE than one thing — deferred then auto-clears only once EVERY named blocker has merged (one of several landing does not unblock a card that needed all of them), and deferredStuck (below) fires if ANY one of them goes dangling/closed-with-no-merge, even while the others are still cleanly pending. A single resolved id (whether you passed one string, a 1-element array, or an array of duplicates) always reads back as a bare string, never a 1-element array. If the blocker instead closes with NO squash commit (a legitimate 0-commit outcome) or is deleted, deferred deliberately does NOT auto-clear (that would silently redefine \"until it merges\" as \"until it's closed\") — instead the read-time `deferredStuck` flag (tasks_list/tasks_get) flips true and the card stops being discounted from the idle watchdog's actionable count, so an unreachable deferral surfaces instead of sitting invisibly forever. Rejects a self-reference or an id that doesn't resolve on this board (whole patch rejected, nothing written). PREFER deferredUntilTaskId over prose whenever the blocker IS a queryable card — a condition named only in deferredReason prose has no auto-clear and can't raise deferredStuck, so a manual deferral whose blocker already resolved can sit indistinguishable from one still genuinely pending (card 022659ac; the specimen that motivated this: a deferral blocked on two cards, both merged within hours, sat deferred for five days because nothing surfaced that). Leave deferredUntilTaskId unset ONLY for a deferral gated on something that genuinely isn't a card — an owner decision, an external/upstream release, an elapsed-time window, an observation like \"if this recurs\" — those must stay manually managed, never auto-cleared, and need a periodic manual sweep rather than expecting a passive flag to be noticed. A MANUAL deferral like that (deferred:true with no deferredUntilTaskId) REQUIRES `deferredReason` (a short string: why it's parked and what would release it) — the write is REFUSED (whole patch rejected, nothing written) if the resulting state would be manually-deferred with no reason recorded either before or after this patch, so a card can never again go byte-identical-to-forgotten (card c90e9525). The daemon also stamps a read-only `deferredAt` (the instant this manual deferral last started — never caller-settable) the first time a reason lands on it; both `deferredAt` and `deferredReason` reset to null on an explicit `deferred:false` clear — and the SAME reset happens on the deferredUntilTaskId auto-release above. Card 1d27c3cd: before either reset, if a reason was recorded it's folded into the card's `body` as its own \"**Previously deferred:**\" paragraph (idempotently replacing any earlier such paragraph, so repeated defer/release cycles never pile up more than the latest one) — so the reasoning survives even though the field itself is cleared; re-read via tasks_get/tasks_list to see it. A route-(a) deferral (deferredUntilTaskId set) needs no reason — its release condition is the named blocker task itself. Optional deferredUntilEvent (card 74716cfb, {kind:\"gate-fail-naming\"|\"request-answered\", key:<string>}) ANNOTATES which event a deferral is watching for — e.g. {kind:\"gate-fail-naming\", key:\"some-test-file.mjs\"} names a gate run whose failed test files include that one — so a gate-outcome nudge that later observes exactly that event can point a reader at this card. It NEVER auto-clears anything (unlike deferredUntilTaskId) and carries no release semantics of its own — release is still your own judgement call after reading deferredReason in full; pass null to clear the annotation. A malformed kind/key is REFUSED (whole patch rejected, nothing written) rather than stored. repoKey (multi-repo epic) re-targets the card to a different entry in this project's `repos` registry, or null/\"primary\" to reset it to the project's primary repo — an unknown key is REFUSED (whole patch rejected, nothing written), same convention as an unknown columnKey. Optional heldRequestId (card 0ad1ca68) is a STANDING annotation — which owner Request (any state, including one already answered/consumed) this card's hold traces back to. Independent of held/deferred and of that Request's own taskId (a request is very often filed with taskId:null, or tied to a different/sibling card) — nothing infers it automatically; set it yourself when you recognize a card's hold explains itself via a specific Request, so a future reader isn't left with only body prose to explain a hold whose originating Request has long since left the pending queue. Must resolve to a real request on THIS project (whole patch rejected otherwise); pass null to clear. tasks_get resolves it LIVE into `heldRequestState` on every read — it survives the linked request moving through pending → answered → consumed, and degrades to a fail-visible {notFound:true} rather than silently dropping if the request is later deleted.\n\nA column/priority/deferred/held/heldRequestId/repoKey/deferredUntilTaskId/deferredReason/deferredUntilEvent-only move needs ONLY id + those fields — no body — and returns a TRIMMED ack ({id,title,columnKey,priority,position,held,deferred,heldBy,heldRequestId,repoKey,deferredUntilTaskId,deferredAt,deferredReason,deferredUntilEvent,updatedAt,version,changed}, no body) instead of echoing the full card back — deferredStuck is read-derived only (not part of this ack); re-read via tasks_get/tasks_list for its current value. Pass body when you're intentionally editing it — that returns the full updated task, body included. ⚠️ `body` is a FULL REPLACE, not a merge or append — whatever string you pass BECOMES the entire body, with NO undo; if you only mean to change part of it, read the current body first (tasks_get) and pass back the whole thing with your edit folded in. DESTRUCTIVE-TRUNCATION GUARD (card 09d68835): a `body` write that would discard the large majority of an existing SUBSTANTIAL body (≥1024 characters, keeping <25% of it) is REFUSED — {error naming the current/proposed lengths, truncation:true, current (the task as it stands right now), currentLength, proposedLength} — instead of silently overwriting (the specimen that motivated this: a one-sentence annotation accidentally replaced a ~13,300-character card). A short body, or a rewrite that stays a comparable size, is never touched by this. If a large discard is genuinely intentional, pass allowTruncate:true to bypass it. The SAME guard (same thresholds, same allowTruncate override, same {truncation:true,...} shape) also covers `deferredReason` (card a53b24ce) — a substantial existing reason replaced by a sliver is refused the same way, even though deferredReason is otherwise a field-only patch needing no baseVersion; pass deferredReason:null to clear one deliberately (never guarded) or allowTruncate:true for a genuinely intentional large discard. ⚠️ NEVER write a placeholder into deferredReason (or body) just to read a task's `version` back — tasks_get and tasks_list (and every tasks_update response) already return `version` directly; there is no read-only way to get it that requires a write.\n\n`appendBody` (card e2756e47) is the ADDITIVE alternative to `body`: appends a timestamped \"## Triage note — <ts>\" section instead of replacing the whole thing — use this for a note, decision, or status update on a card whose body already carries something worth keeping (e.g. a worker's report or another agent's analysis), so your note can never clobber it the way a `body` write would. Passing both `body` and `appendBody` is REJECTED (whole patch, nothing written) — they're different intents. `appendBody` needs NO `baseVersion` (unlike `body`): an append can't destructively clobber a concurrent edit the way a full replace can, so there's nothing to gate — worst case under a race is two sections landing in a nondeterministic order, never lost content. The destructive-truncation guard above still applies to the resulting write (it can never fire on a correct append, but stays as a backstop against a buggy one). Returns the full updated task, same as a `body` write.\n\nOPTIMISTIC CONCURRENCY (card d0978321): writing `title` and/or `body` on an EXISTING task REQUIRES `baseVersion` — the `version` you last read for this task (from tasks_get/tasks_list/a prior tasks_update response). A stale-or-omitted `baseVersion` on a title/body write is REJECTED with {error, conflict:true, current} (the task as it stands right now) instead of overwriting — re-read it and retry, merging your change into the current body. This is a PLAIN COMPARISON against the task's current `version`, not a separate read-tracker (card fe4a9a17) — the error text says which of the two actual causes fired: `baseVersion` omitted entirely (by far the more common real cause — you forgot the param, not a race), or a genuinely stale value (`current` differs from what you last read). If `current` is byte-identical to what you already have, nobody wrote it in between (mirrors memory_write exactly). A column/priority/held/deferred/position/repoKey/deferredUntilTaskId/deferredReason/deferredUntilEvent-only patch needs NO baseVersion at all — those never touch prose and are never gated. `version` ADVANCES ONLY when title/body actually change — a field-only move leaves it unchanged, so an unchanged version does NOT mean the card is unchanged in every sense, only that its title/body haven't.\n\nHTML-ENTITY GUARD (card 267fd215): a `title` write containing &lt;/&gt;/&amp;/&quot;/&#NN; is REFUSED with {error} naming the decoded form you likely meant — a SOLO merge uses the title VERBATIM as the squash commit subject, so an accidentally-escaped title becomes a PERMANENT mainline artifact. If the title is genuinely ABOUT escaped HTML (not an accidental artifact), pass allowHtmlEntities:true; irrelevant when this patch doesn't touch `title`.\n\nCONVENTIONAL-TYPE GUARD (card 3a833d94): a `title` write carrying a `type(scope):`-shaped prefix whose type is NOT one of feat/fix/docs/style/refactor/perf/test/build/ci/chore/revert is REFUSED with {error} naming the bad type and the allowed set — the merge-time coercion net only fixes BARE prose (no type-shaped prefix at all), so an already type-shaped-but-invalid prefix would otherwise ship verbatim as a permanent mainline commit subject. A title with no type-shaped prefix at all is unaffected. Pass allowNonConventionalType:true if this is deliberate; irrelevant when this patch doesn't touch `title` — an existing card whose STORED title already carries an invalid type is never retroactively blocked by an unrelated field-only patch.\n\nTERMINAL-LANE PENDING-REQUEST WARNING (card c4355598): a `columnKey` move into the board's terminal (done) lane is ADDITIVELY flagged with `pendingRequestWarning: [{id, title}, ...]` when this task still has one or more `state:\"pending\"` owner Requests connected to it (the same connected-requests set task_requests_list/task_request_get read) — never a bare count, always the specific request id(s) + their actual question title(s). This NEVER blocks the move — grounded in a specimen, not a preference: card 3b2aa339 is a terminal close that was CORRECT despite a still-pending Request (it shipped one option and deliberately left another to the owner), so refusing the move would have blocked a correct action — and the field is OMITTED entirely for a non-terminal move or when nothing is pending.",
          inputSchema: strictShape({
            ...
            deferredUntilTaskId: z.union([z.string(), z.array(z.string())]).nullable().optional(),
            deferredReason: z.string().nullable().optional(),
            deferredUntilEvent: z.object({ kind: z.enum(["gate-fail-naming", "request-answered"]), key: z.string() }).nullable().optional(),
            repoKey: z.string().nullable().optional(),
            heldRequestId: z.string().nullable().optional(),
        ...
        async ({ id, taskId, baseVersion, allowTruncate, appendBody, allowHtmlEntities, allowNonConventionalType, ...patch }) => {
          ...
          const result = await updateProjectTask(db, projectId, resolvedId, patch, { sessionId, role: session?.role }, baseVersion, allowTruncate, appendBody, allowHtmlEntities, allowNonConventionalType);
