    ...
    proofs: [
      {
        ...
        region: "whole file",
        pattern: `const runBuild = !noCommit`,
        note: "Worker provisioning (createWorktree) skips the build phase for an ordinary noCommit rig (a review spawn additionally builds when the reviewed diff needs it — card 503cd822) — a session-lifecycle read, not spawn argv, so it applies identically regardless of harness.",
      },
    ],
  },
};
