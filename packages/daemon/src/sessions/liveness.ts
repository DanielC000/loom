...
export function sweepDeadSessions(db: Db): number {
  ...
  for (const s of db.listResumeCandidates()) {
    if (s.engineSessionId && !engineTranscriptExists(s.cwd, s.engineSessionId, s.harness)) {
 ...
 * code — this is the first real consumer, mirroring `claudeAdapter`'s own usage above exactly.
 *
 * UPDATE (card 2ec60d9c): the "near-no-op" gap this doc originally called out is closed —
 * `pty/host.ts#captureCodexEngineSessionId` now discovers + reports a codex session's `engineSessionId`
 * (via the SAME `onEngineSessionId` event claude's SessionStart-hook branch fires), and
 * `sweepDeadSessions`'s own `engineTranscriptExists(s.cwd, s.engineSessionId, s.harness)` call now threads
 * that session's `harness` through to the harness-aware resolution seam (`sessions/transcript.ts`) instead
 * of always resolving against claude's own JSONL path scheme. So this watcher is now a genuine, working
 * dead-session detector for codex, not just wiring waiting on a future capture mechanism.
 */
