...
// @decision a9119abf — that split-not-override remedy is SUPERSEDED, only for files with an OBSERVED kill (the
...
const TEST_TIMEOUT_OVERRIDES = {
  ...
  "merge-gate-inert-diff": 300_000, // cards e5a75b65/55ea3b32: already in ISOLATED_REAL_SPAWN_BASENAMES below ("11x Db/SessionService boot, 15x real createWorktree"); at authoring (17d03ac1^) it was one of SIX class members carrying no override — kickoff-real-spawn, merge-gate-inert-diff, emit-compare-gate, gate-status, merge-confirm-completion-nudge, merge-spawn-tracked — not the only one (card bfbe616b). It ran on the blanket 120s ceiling. Per-file history (~/.loom/gate-timing/daemon-per-file-timing.ndjson), n=15: 14 passes at 71,229-85,432ms (median 75,189ms, max 85,432ms) and 1 fail SIGTERM-killed AT the 120,000ms ceiling (censored — true cost unknown, bounded only from below, not a duration). Margin at the observed max pass vs the 120s ceiling: 1.40x. A same-window sibling gate ran this file concurrently and PASSED at 82,476ms, refuting concurrent load as the discriminating cause (present in both the failing and a passing run) — the honest attribution is a thin margin plus ordinary variance, not a race. 300k gives 3.51x margin at the observed max, matching the proactive-buffer posture already granted to merge-canonical-dirty-overlap-backstop/merge-canonical-untracked-overlap-backstop above (measured 16.8s/12.6s standalone).
  "gate-status": 300_000, // card bfbe616b: same ISOLATED_REAL_SPAWN_BASENAMES class as merge-gate-inert-diff above, one of the six members untreated at 17d03ac1^ and STILL untreated today — carried no override despite a margin thinner than merge-gate-inert-diff's own pre-override 1.40x. Per-file history (~/.loom/gate-timing/daemon-per-file-timing.ndjson), the 14 full-suite runs 2026-09-12->2026-09-13 that carry per-file rows: 14/14 passes, 79,914-90,303ms (median 83,173ms, max 90,303ms), 0 fails. Margin at the observed max pass vs the 120s ceiling: 1.33x. Its max pass (90,303ms) landed within 97ms of merge-gate-inert-diff's own max pass (90,206ms) in the SAME contended window (2026-09-12T15:10Z vs 15:14Z) — two files with an indistinguishable duration profile carrying a 2.5x difference in headroom before this entry. 300k gives 3.32x margin at the observed max, matching the posture already granted to merge-gate-inert-diff above (3.33x).
  "emit-compare-gate": 300_000, // card bfbe616b: also one of the six ISOLATED_REAL_SPAWN_BASENAMES members untreated at 17d03ac1^ and still untreated today. Per-file history (~/.loom/gate-timing/daemon-per-file-timing.ndjson), 16 real passes 2026-09-12->2026-09-13 (14 from full-suite runs, 2 from smaller targeted runs the same day): 56,276-75,257ms (median 68,066ms, max 75,257ms — in the SAME contended window as gate-status/merge-gate-inert-diff's own maxima), 0 fails. Margin at the observed max pass vs the 120s ceiling: 1.59x — only 14% above the 1.40x margin merge-gate-inert-diff actually failed at once (SIGTERM, pre-override), and materially thinner than this file's own untreated siblings measured comfortably safe (kickoff-real-spawn 2.39x, merge-spawn-tracked 2.95x — see the no-override decision above). 300k gives 3.99x margin at the observed max.
  // card a9119abf — the nine entries below share one basis: 7 full-suite runs with per-file rows, all
  // 2026-09-24 on one host (n=7 per file, one day, one host — not a rate); a kill is censored at 120,000ms,
  // so max pass understates. Margin = 300k / observed max pass (or the sibling's, where marked). Cutoff:
  // an observed kill, or a max pass >= 108,000ms (0.9x the blanket ceiling).
  "batch-merge": 300_000, // card a9119abf: n=7, max pass 111,133ms, 2 kills (SIGTERM at the ceiling); 2.70x margin.
  "batch-merge-robustness": 300_000, // card a9119abf: n=7, max pass 112,887ms, 2 kills; 2.66x margin.
  "batch-merge-robustness-redundancy": 300_000, // card a9119abf: n=1, 101,942ms in-suite (82s in a targeted run). Carries the override because it is a 4e8e2d82 split half of batch-merge-robustness (2 kills) — inherited risk, not a risk measured for this file itself; 2.94x margin.
  "batch-merge-gate-history": 300_000, // card a9119abf: n=7, max pass 119,740ms, 1 kill; 2.51x margin.
  "batch-merge-reduced-gate": 300_000, // card a9119abf: n=7, max pass 113,101ms, 1 kill; 2.65x margin.
  "batch-merge-gate-retry": 300_000, // card a9119abf: n=7, max pass 108,380ms, 1 kill; 2.77x margin.
