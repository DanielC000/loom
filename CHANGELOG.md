...
- **A fact-check pass corrected the claims that refresh got wrong**, followed by an independent re-check that caught more. The remote-access story was the worst of it and is now described as it actually behaves: **Tailscale Serve is no longer recommended** (the CSRF guard refuses a `.ts.net` `Origin` before any other check, so writes and every WebSocket 403 through it), an SSH port-forward is the single recommended path with its one-time token step and what failure looks like before you do it, the direct bind is described as an authenticated **API rather than a cockpit** (the web app's own routes aren't remotely reachable) with its `bindHost: "0.0.0.0"` 403 and its effect on local `loom status/stop/open` spelled out, and every mutating `curl` on the page now carries the `Authorization: Bearer` header it has needed since 0.28.0. Elsewhere: the Gates page has no kill switch on a run; the Elevated Operator's "never a force-push" binds its MCP tool, not the session's own shell; revoking a delivered credential is de-provisioning, not containment, and doesn't reach a running session; the decision-record store is scaffolded only for a project Loom creates, not one you bind; the skills editor three-way-merges `SKILL.md` while other files get a per-file take-mine/take-shipped; the Codex harness is worker-shaped by doctrine rather than by enforcement; the Platform operator archives projects but not agents or profiles; enabling the scheduler needs a daemon restart; and the site quickstart's from-source line now builds before it runs the daemon.
