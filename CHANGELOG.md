...
### Added
- **Tailscale Serve (and other same-host reverse proxies) are now supported as a read-and-steer cockpit — not yet verified against a live Tailscale node.** A human-only `remoteAccess.proxyPort` opens a third listener bound to `127.0.0.1` only, and a human-only `remoteAccess.trustedProxyOrigins` names the exact origins the proxy fronts. Every request on that port is remote-class whatever its `Host` — gateway token required, Tier-1 surface only, view-only terminals, `/internal/*` and the local access credential unreachable — because on the two extra listeners (this one and the remote one) the trust class follows the listener a request arrived on, decided in one place, rather than a peer address or a header a proxy may rewrite; on the daemon's own port it still follows the peer, so a proxy must never be pointed there. The cockpit's static files are served to that class without a token; the browser keeps the gateway token in its own per-origin key (`?gwtoken=`) with its own banner, separate from the loopback credential. Off by default; needs `remoteAccess.enabled`, a gateway token and a restart; a `Tailscale-Funnel-Request` is refused on every route of the proxy listener. The proxy must preserve the original `Host`. Tested only against a minimal in-test Node reverse proxy over plain HTTP. See the Remote access docs.
...
### Changed
- **Anything arriving on the remote listener is now remote-class, even from a loopback peer.** Before, a same-host connection to the remote port was trusted as loopback (no token, loopback-secret writes); it now needs a gateway token and reaches only the Tier-1 surface, like any remote caller. The daemon's own loopback port is unchanged.
...
- **A fact-check pass corrected the claims that refresh got wrong**, followed by an independent re-check that caught more. The remote-access story was the worst of it and is now described as it actually behaves: **Tailscale Serve was described as unsupported** (the CSRF guard refuses a `.ts.net` `Origin` before any other check, so writes and every WebSocket 403 through it) — superseded later in this same release by the trusted-proxy listener above, an SSH port-forward is the single recommended path with its one-time token step and what failure looks like before you do it, the direct bind is described as an authenticated **API rather than a cockpit** (the web app's own routes aren't remotely reachable) with its `bindHost: "0.0.0.0"` 403 and its effect on local `loom status/stop/open` spelled out, and every mutating `curl` on the page now carries the `Authorization: Bearer` header it has needed since 0.28.0. Elsewhere: the Gates page has no kill switch on a run; the Elevated Operator's "never a force-push" binds its MCP tool, not the session's own shell; revoking a delivered credential is de-provisioning, not containment, and doesn't reach a running session; the decision-record store is scaffolded only for a project Loom creates, not one you bind; the skills editor three-way-merges `SKILL.md` while other files get a per-file take-mine/take-shipped; the Codex harness is worker-shaped by doctrine rather than by enforcement; the Platform operator archives projects but not agents or profiles; enabling the scheduler needs a daemon restart; and the site quickstart's from-source line now builds before it runs the daemon.
- **The "nothing leaves your machine" claim is now an honest list.** The README and the site said Loom adds no cloud service of its own and left the reader to conclude nothing goes out by default — which was wrong in three ways: a plan-usage poll to `api.anthropic.com` every 60s carrying your Claude OAuth token, an update check against `registry.npmjs.org` every 6h on an npm-installed daemon, and the package-manager install that runs when a worker's worktree is created. All three are now named, with the off switch where one exists (`LOOM_SUPPRESS_USAGE_POLLER=1`) and the plain admission where none does. What Loom never does — upload your code, transcripts, or history, or run a service of its own — is stated as that, rather than as an absolute about all outbound traffic.
