...
| `doctrineInjection` | `"directory"` (`.claude/skills`) | `"file"` | **BUILT (card `887e10b8`), stale row corrected.** `injectCodexDoctrine` (`pty/codex-doctrine.ts`) writes a condensed, hand-authored worker doctrine into `<cwd>/AGENTS.md` before `createCodexPty` spawns — role-gated to `worker` only (manager/platform/auditor/workspace-auditor/setup get zero doctrine injection on codex today, a named Phase-1 scope limit, not an oversight). It is a condensed transcription of the load-bearing rules from claude's `/worker` skill (targeted-test default, no speculative full-gate, escalate-up), not a full port — codex has no skill-invocation tool to point at a larger body the way claude does. `injectSkills` is never called for codex at all (`@decision 7fbd1ba5`) — deliberate, not a gap. |
