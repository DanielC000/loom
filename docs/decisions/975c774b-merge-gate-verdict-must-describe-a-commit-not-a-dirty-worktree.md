...
`mergeBranchLocked` resolves the branch tip LIVE at squash time (@decision 7efc2bff) and the gate-ran path passed it no gated tip, so a commit landing mid-gate (clean tree, so no dirt stamp) rode the PASS onto main. `captureGatedTip`'s `gatedTip` was read only as the cache stamp. Now, after the 975c774b dirt check and before `beginSquash`, a PASS re-resolves the branch and refuses on a mismatch or an unreadable tip (fail closed); the same `gatedTip` is passed to `mergeBranch` as `expectedBranchTip`, which refuses inside the lock with zero side effects (`branchTipMoved`). Both surface as `gateTipMoved:{phase,gated,live}`. 8b1fb28f's cache-identity semantics are untouched: a clean head move is still a cache-identity mismatch on a later call, and this refusal only fires on a PASS at settle, uncached, so the re-call re-gates the new tip. Not exposed: the batch path (`git/batch-merge.ts`) cherry-picks each candidate's tip into its own batch worktree, gates that, and fast-forwards canonical to the gated batch head, so a later commit on a candidate branch cannot ride in. Tests: `merge-confirm-gated-tip-squash.mjs` (pre-check via the service, in-lock via a direct `mergeBranch` call — the window between the two is not deterministically reachable through the service). `merge-gate-reuse-admission.mjs` scenario (M) now meets the gated-tip refusal first (its branch moves mid-gate); scenario (N) still isolates `requireCanonicalHead`.
