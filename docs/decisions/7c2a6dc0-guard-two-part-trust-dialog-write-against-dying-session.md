...
- Do not capture `submitCancelGen` only after `submitCodex`'s chunked write completes — a stop/redirect landing mid-chunk would then have nothing to compare against, and the delayed-Enter closure could write a stray `"\r"` into an already-stopped or redirected session.
- Do not guard `submitCodex`'s delayed Enter with `busyStaleGen`. That counter also moves on every busy-marker re-arm (`armCodexBusyStaleTimer`), and codex's title spinner animates while it starts MCP servers, so a marker chunk in the text->Enter gap used to cancel the Enter silently (card `ea97817c`: `enterPending` stuck true, the retry ladder's CASE 0 no-op'd forever, prompt left in the composer). `submitCancelGen` is bumped ONLY by `stopCodex` and `interruptForRedirectCodex`, next to (never instead of) their own `busyStaleGen++`, which the staleness timer's CASE 1 still relies on.
