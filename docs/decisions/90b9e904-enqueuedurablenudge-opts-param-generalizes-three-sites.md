...
JSDoc comment in `packages/daemon/src/sessions/service.ts`, above `enqueueDurableNudge`: lines 4380-4394, as of this tranche's HEAD (tranche 10). Second citing site (no new content): JSDoc comment above `recoverCrashOrphanedWorkers`, line 4820, as of this tranche's HEAD (tranche 14). Third citing site (no new content): the `CrashRecoveryWatcher` class doc in `packages/daemon/src/orchestration/crash-recovery-watcher.ts`, lines 237-239, as of that file's own tranche 1 HEAD. Fourth citing site (no new content): inline comment above the dispatch branch in `WakeService.tick()`, `packages/daemon/src/orchestration/wake.ts`, originally lines 221-222, as of this tranche's HEAD.
