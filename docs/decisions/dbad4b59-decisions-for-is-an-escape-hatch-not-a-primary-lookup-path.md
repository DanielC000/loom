...
Deliberately NOT Loom-source-specific, unlike `comment-anchor-lint.mjs` (which, when this was written, was a Loom-internal lint tool with a hardcoded `SOURCE_ROOTS`; card 03fbb126 made it layout-agnostic too). This tool runs against ANY project's repo via `TaskMcpRouter`'s server-derived `repoPath`, so its file walk is repo-root-relative and layout-agnostic (mirrors `repo-read.ts`'s own generic `walkFiles`), never assuming a fixed `packages/<pkg>/src` layout.
