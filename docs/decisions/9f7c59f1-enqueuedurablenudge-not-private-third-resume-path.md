...
JSDoc comment in `packages/daemon/src/sessions/service.ts`, above `enqueueDurableNudge`: lines 4371-4379, as of this tranche's HEAD (tranche 10). Second section's source: JSDoc comment above `resumeFleetOnBoot`, lines 4316-4335 and 4369-4375, as of this tranche's HEAD (tranche 11). Third citing site (no new content): JSDoc comment above `recoverCrashOrphanedWorkers`, lines 4818-4822, as of this tranche's HEAD (tranche 14). Fourth and fifth citing sites, in `packages/daemon/src/orchestration/crash-recovery-watcher.ts`: the `CrashRecoveryDeps.enqueueDurableNudge` field doc (lines 76-91 — carries one piece of new mechanism detail kept inline there rather than duplicated here: the specific `PtyHost.waitForMcpSeen`/`usesOrchestrationMcp` gates the raw call lacked) and the `CrashRecoveryWatcher` class doc (lines 232-243, no new content), both as of crash-recovery-watcher.ts's own tranche 1 HEAD.
