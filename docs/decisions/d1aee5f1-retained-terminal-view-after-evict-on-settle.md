...
## The merge op's own `classifyOutcome`, follow-up

`confirmWorkerMergeTracked` opts in with a distinct terminal outcome per settle, so the Board's merge-gate
hairline can render merged/rejected/cancelled/(failed via the raw `state` field) instead of reverting the
instant the gate settles: `merged:true` → `"merged"`; `cancelled:true` (checked BEFORE the merged/rejected
branch, since a cancelled outcome also carries `merged:false` and would otherwise misclassify as
`"rejected"`) → `"cancelled"`; a resolved `merged:false` with no `cancelled` flag (the gate/stranded-work/
empty-stage rejection paths — none of these throw) → `"rejected"`; a genuinely thrown error → `"unknown"`,
never `"failed"` ([[479f449f-pendingmerge-outcome-unknown-is-never-failed]]).
...
- Do not classify a cancelled outcome AFTER the merged/rejected check — it also carries `merged:false` and
  would otherwise misclassify as `"rejected"`; check `cancelled` first.
