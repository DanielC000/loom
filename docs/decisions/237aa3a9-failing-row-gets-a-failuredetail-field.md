...
2. **`failureType` as a first-class field** — a bare message forces every reader to parse prose; a type (at minimum: assertion failed / test code threw / timeout-kill / nonzero exit with no parsed cause) lets a reader route in one glance. Four buckets beat twelve; an honest "unclassified" bucket beats a wrong label.
